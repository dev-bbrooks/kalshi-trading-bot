...
from db import (
    ...
    get_recent_trades, get_trade, update_trade,
    delete_trades, recompute_all_stats,
    get_trade_summary, get_lifetime_stats,
    ...
    insert_log,
    get_btc_candles, get_push_log,
    get_skip_analysis,
    get_all_hourly_stats,
    get_conn,
    insert_audit_log, backup_database,
)
...
def requires_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if auth and check_auth(auth.username, auth.password):
            ...
            resp.set_cookie("bot_auth", _auth_token(), max_age=30*86400,
...
def api_logout():
    resp = jsonify({"ok": True})
    resp.set_cookie("bot_auth", "", max_age=0)
    return resp
...
def api_state():
    ...
    try:
        tm = get_config("trading_mode", "")
        if not tm:
            # Migration: derive from legacy booleans
            obs = get_config("observe_only", False)
            shd = get_config("shadow_trading", False)
            auto = get_config("auto_strategy_enabled", False)
            if obs and shd:
                tm = "shadow"
            elif obs:
                tm = "observe"
...
def _get_anthropic_key() -> str:
    """Get Anthropic API key: check bot_config first, fall back to env."""
    stored = get_config("anthropic_api_key", "")
    if stored:
        return stored.strip()
...
def api_apikey_status():
    """Return masked version of the current API key and its source."""
    stored = get_config("anthropic_api_key", "")
    ...
    if stored:
        ...
        return jsonify({"set": False, "source": None, "preview": ""})

    if len(key) > 12:
        preview = key[:8] + "…" + key[-4:]
    else:
        preview = key[:4] + "…"
    return jsonify({"set": True, "source": source, "preview": preview})
...
def api_command():
    ...
    if cmd not in ("start", "stop", "cash_out", "reset_streak",
                   "update_config", "lock_bankroll", "reset_session", "recover_session",
                   "dismiss_summary", "cancel_cash_out"):
        return jsonify({"error": "Invalid command"}), 400

    # Lock/unlock: save config immediately so UI updates fast,
    # also queue command so bot updates
    ...
    # Reset session: handle immediately, no need to queue
    if cmd == "reset_session":
        state = get_bot_state()
        sw = state.get("session_wins", 0) or 0
        sl = state.get("session_losses", 0) or 0
        # Save prev session if it has data
        if sw + sl > 0:
            import json as _json
            prev = _json.dumps({"wins": sw, "losses": sl,
                "pnl": state.get("session_pnl", 0) or 0,
                "skips": state.get("session_skips", 0) or 0,
                "saved_at": state.get("last_updated", "")})
            update_bot_state({"_prev_session": prev})
        update_bot_state({
    ...
    # Recover session: handle immediately
    if cmd == "recover_session":
        import json as _json
        state = get_bot_state()
        prev_raw = state.get("_prev_session", "")
        if prev_raw:
            prev = _json.loads(prev_raw) if isinstance(prev_raw, str) else prev_raw
            update_bot_state({
                "session_wins": (state.get("session_wins", 0) or 0) + (prev.get("wins", 0) or 0),
                "session_losses": (state.get("session_losses", 0) or 0) + (prev.get("losses", 0) or 0),
                "session_pnl": (state.get("session_pnl", 0) or 0) + (prev.get("pnl", 0) or 0),
                "session_skips": (state.get("session_skips", 0) or 0) + (prev.get("skips", 0) or 0),
                "_prev_session": "",
            })
            return jsonify({"ok": True, "recovered": True})
        return jsonify({"ok": True, "recovered": False})

    # Reset streak: handle immediately
    if cmd == "reset_streak":
        update_bot_state({"loss_streak": 0, "cooldown_remaining": 0})
        return jsonify({"ok": True})
...
def api_trades_v2():
    ...
    if "all" not in filter_list and filter_list:
        ...
        for f in filter_list:
            if f == "win": conditions.append("outcome = 'win'")
            elif f == "loss": conditions.append("outcome = 'loss'")
            elif f == "cashed_out": conditions.append("outcome = 'cashed_out'")
            elif f == "skipped": conditions.append("outcome IN ('skipped', 'no_fill', 'error')")
            elif f == "error": conditions.append("outcome = 'error'")
            elif f == "incomplete": conditions.append("outcome = 'skipped' AND market_result IS NULL")
            elif f == "ignored": conditions.append("COALESCE(is_ignored, 0) = 1")
            elif f == "shadow": conditions.append("COALESCE(is_shadow, 0) = 1")

            elif f == "yes": conditions.append("side = 'yes' AND outcome IN ('win','loss','cashed_out','open')")
            elif f == "no": conditions.append("side = 'no' AND outcome IN ('win','loss','cashed_out','open')")
        if conditions:
    ...
    with get_conn() as c:
        ...
        stats_row = c.execute(f"""
            SELECT
                ...
                MAX(CASE WHEN outcome IN ('win','loss','cashed_out') THEN pnl ELSE NULL END) as best,
                MIN(CASE WHEN outcome IN ('win','loss','cashed_out') THEN pnl ELSE NULL END) as worst
...
def api_reset():
    ...
    try:
        if scope == "settings":
            with get_conn() as c:
                keep = ('locked_bankroll', 'push_subscription', 'push_vapid_public',
            ...
            with get_conn() as c:
                ...
                c.execute("DELETE FROM market_observations")
                ...
                try:
                    c.execute("DELETE FROM btc_probability_surface")
                    c.execute("DELETE FROM feature_importance")
                    c.execute("DELETE FROM regime_stability_log")
                except Exception:
                    pass
            recompute_all_stats()
            update_bot_state({"active_trade": None, "active_skip": None, "active_shadow": None,
            ...
            set_config("regime_filters", {})
            set_config("regime_overrides", {})
            ...
            with get_conn() as c:
                ...
                try:
                    ...
                    pass
            update_bot_state({"regime_engine_phase": None})
            return jsonify({"ok": True, "scope": "regime_engine", "msg": "Regime engine data wiped"})

        elif scope == "session":
            update_bot_state({
                "session_pnl": 0, "session_wins": 0, "session_losses": 0,
                "session_skips": 0,
                "loss_streak": 0,
                "cooldown_remaining": 0, "_prev_session": "",
            })
            return jsonify({"ok": True, "scope": "session", "msg": "Session stats reset"})

        elif scope == "locked":
            set_config("locked_bankroll", 0)
            set_config("profit_goal_reached", False)
            update_bot_state({
                "loss_streak": 0,
            })
            return jsonify({"ok": True, "scope": "locked", "msg": "Locked bankroll zeroed"})
            ...
            set_config("locked_bankroll", 0)
            set_config("profit_goal_reached", False)
            return jsonify({"ok": True, "scope": "soft", "msg": "Session and locked bankroll reset"})
            ...
            update_bot_state({
                ...
                "last_ticker": None, "_prev_session": "",
                "active_skip": None,
                "active_shadow": None,
...
def api_skip_conditions():
    ...
    # Session target
    starget = float(cfg.get("session_profit_target", 0))
    if starget > 0:
        conditions.append({"type": "session_target", "label": f"Session target: ${starget:.0f}",
                           "color": "dim"})

    # Session loss limit
    slimit = float(cfg.get("session_loss_limit", 0))
    if slimit > 0:
        conditions.append({"type": "session_loss", "label": f"Session loss limit: -${slimit:.0f}",
                           "color": "orange"})
...
def api_recompute_strategies():
    ...
    try:
        ...
        with get_conn() as c:
            count = c.execute("SELECT COUNT(*) as n FROM market_observations WHERE market_result IS NOT NULL").fetchone()["n"]
        # Clear discovery cache so new ones fire
        set_config("_strategy_discoveries_notified", "[]")
        ...
        if processed > 0:
            with get_conn() as c:
                stale = c.execute("DELETE FROM strategy_results WHERE updated_at < ?", (_before,))
...
def api_walkforward_selection_reset():
    ...
    try:
        from db import set_config
        set_config("_selection_test_result", "")
...
def api_validation_summary():
    ...
    try:
        ...
        selection_test_result = get_config("_selection_test_result") or ""
...
@requires_auth
def api_arcade():
    """Serve game content from game.py."""
    try:
        from game import GAME_HTML
        return GAME_HTML, 200, {"Content-Type": "text/html"}
    except ImportError:
        return "<div style='padding:20px;text-align:center;color:var(--dim)'>No game loaded</div>", 200, {"Content-Type": "text/html"}
    except Exception as e:
        return f"<div style='padding:20px;color:var(--red)'>Error: {e}</div>", 200, {"Content-Type": "text/html"}
...
def api_regime_detail(label):
    ...
    with get_conn() as c:
        ...
        from db import get_config
        regime_filters = get_config("regime_filters", {})
        if isinstance(regime_filters, str):
            import json as _json
...
def api_llm_summary():
    ...
    sl = state.get('session_losses', 0) or 0
    st = sw + sl
    swr = round(sw / st * 100) if st > 0 else 0
    lines.append(f"- Session: {sw}W-{sl}L ({swr}% WR), P&L: {fpnl(state.get('session_pnl', 0))}, skips: {state.get('session_skips', 0)}")
    lines.append("")
...
def api_server_control():
    ...
    services = ["kalshi-bot", "kalshi-dashboard"] if service == "all" else [service]
    valid = {"kalshi-bot", "kalshi-dashboard"}
...
def api_system_stats():
    ...
    try:
        ...
        # Database file size
        db_path = os.path.join(os.environ.get("BOT_DIR", "/opt/15-min-btc-bot"), "botdata.db")
        ...
        # Log file size
        log_path = os.path.join(os.environ.get("BOT_DIR", "/opt/15-min-btc-bot"), "bot.log")
...
def _gather_chat_context():
    ...
    for k in ['status', 'status_detail', 'auto_trading', 'loss_streak',
    ...
    cfg = get_all_config()
    parts.append("\n## Config")
    _sensitive_keys = {"anthropic_api_key", "push_vapid_private", "push_vapid_public",
                       "push_subscription", "dashboard_password",
                       "sell_target_pct", "custom_sell_price_c",
                       "push_notify_observed_outcomes",
                       "entry_price_min_c", "fill_wait_seconds", "min_minutes_left"}
    ...
    _risk_display = lambda r: 'extreme' if r == 'terrible' else r
    for r in regimes:
        wr = f"{(r.get('win_rate',0)*100):.1f}%"
        rl = _risk_display(r.get('risk_level','?'))
        parts.append(f"  {r['regime_label']}: {rl} risk, "
                     f"{wr} win rate, {r.get('total_trades',0)} trades, "
                     f"P&L {fpnl(r.get('total_pnl',0))}, "
                     f"CI [{(r.get('ci_lower',0)*100):.0f}-{(r.get('ci_upper',1)*100):.0f}%]")

    # Active regime filters
    regime_filters = cfg.get("regime_filters", {})
    if isinstance(regime_filters, str):
        import json as _j
        regime_filters = _j.loads(regime_filters)
    if regime_filters:
        ...
        for label, rf in regime_filters.items():
            filter_parts = []
            if rf.get("blocked_hours"): filter_parts.append(f"hours blocked: {rf['blocked_hours']}")
            if rf.get("blocked_days"): filter_parts.append(f"days blocked: {rf['blocked_days']}")
            if rf.get("vol_min", 1) > 1 or rf.get("vol_max", 5) < 5:
                filter_parts.append(f"vol {rf.get('vol_min',1)}-{rf.get('vol_max',5)}")
            if rf.get("stability_max", 0) > 0: filter_parts.append(f"stability max {rf['stability_max']}c")
            if rf.get("blocked_sides"): filter_parts.append(f"blocked sides: {rf['blocked_sides']}")

            if rf.get("max_spread_c", 0) > 0: filter_parts.append(f"max spread {rf['max_spread_c']}c")
            if filter_parts:
                parts.append(f"  {label}: {', '.join(filter_parts)}")

    # Recent trades (last 30) — include data bet flag and cost
    ...
    for t in trades:
        ...
        entry = t.get('avg_fill_price_c') or t.get('entry_price_c') or 0
        pnl = t.get('pnl', 0)
        is_data = bool(t.get('is_data_collection', 0))
        cost = t.get('actual_cost', 0) or 0
        bet_type = "DATA-BET(1 share)" if is_data else f"NORMAL"
        parts.append(f"  #{t['id']} {t.get('outcome','?')} {side}@{entry}c "
                     f"PnL={fpnl(pnl)} cost=${cost:.2f} "
                     f"type={bet_type} "
    ...
    # Strategy Observatory — simulation results
    try:
        from db import get_conn, rows_to_list, get_observation_count

        obs_counts = get_observation_count()
        parts.append(f"\n## Strategy Observatory")
        parts.append(f"  Markets recorded: {obs_counts.get('total', 0)}")
        parts.append(f"  Outcomes resolved: {obs_counts.get('resolved', 0)}")
        parts.append(f"  Traded: {obs_counts.get('traded', 0)}")
        parts.append(f"  Observed: {obs_counts.get('observed', 0)}")

        with get_conn() as c:
            # Top 15 global strategies by EV
            top_global = c.execute("""
                SELECT strategy_key, side_rule, exit_rule, entry_time_rule,
                       entry_price_max, sample_size, wins, losses, win_rate,
                       total_pnl_c, avg_pnl_c, profit_factor, ci_lower, ci_upper,
                       ev_per_trade_c, max_consecutive_losses, max_drawdown_c
                FROM strategy_results
            ...
            if top_global:
                parts.append(f"\n## Top Global Strategies (best EV, n≥10)")
                parts.append("  Columns: side | exit | timing | maxPrice | n | WR | EV/trade | CI | PF | maxLoss | totalPnL")
                for s in top_global:
                    wr = f"{(s['win_rate'] or 0)*100:.0f}%"
                    ci = f"{(s['ci_lower'] or 0)*100:.0f}-{(s['ci_upper'] or 1)*100:.0f}%"
                    ev = f"{(s['ev_per_trade_c'] or 0):+.1f}¢"
                    pf = f"{s['profit_factor']:.2f}" if s.get('profit_factor') else '—'
                    pnl = f"{(s['total_pnl_c'] or 0)/100:+.2f}"
                    parts.append(f"  {s['side_rule']} | {s['exit_rule']} | {s['entry_time_rule']} | "
                                 f"≤{s['entry_price_max']}¢ | n={s['sample_size']} | {wr} | {ev} | "
                                 f"CI {ci} | PF {pf} | maxL={s.get('max_consecutive_losses',0)} | ${pnl}")

            # Bottom 10 (worst EV) to know what to avoid
            bottom_global = c.execute("""
                SELECT strategy_key, side_rule, exit_rule, entry_time_rule,
                       entry_price_max, sample_size, win_rate, ev_per_trade_c,
                       ci_lower, total_pnl_c
                FROM strategy_results
                WHERE setup_key = 'global:all' AND sample_size >= 10
            ...
            if bottom_global:
                parts.append(f"\n## Worst Global Strategies (avoid these)")
                for s in bottom_global:
                    wr = f"{(s['win_rate'] or 0)*100:.0f}%"
                    ev = f"{(s['ev_per_trade_c'] or 0):+.1f}¢"
                    pnl = f"{(s['total_pnl_c'] or 0)/100:+.2f}"
                    parts.append(f"  {s['side_rule']} | {s['exit_rule']} | {s['entry_time_rule']} | "
                                 f"≤{s['entry_price_max']}¢ | n={s['sample_size']} | {wr} | {ev} | ${pnl}")

            # Per-regime best strategies (top 3 per coarse regime, only regimes with data)
            regime_strats = c.execute("""
                SELECT setup_key, strategy_key, side_rule, exit_rule, entry_time_rule,
                       entry_price_max, sample_size, wins, losses, win_rate,
                       ev_per_trade_c, ci_lower, ci_upper, profit_factor,
                       total_pnl_c, max_consecutive_losses
                FROM strategy_results
            ...
            if regime_strats:
                parts.append(f"\n## Best Strategy Per Regime (top 3 each, n≥5)")
                current_setup = None
                count = 0
                for s in regime_strats:
                    ...
                    pf = f"{s['profit_factor']:.2f}" if s.get('profit_factor') else '—'
                    pnl = f"{(s['total_pnl_c'] or 0)/100:+.2f}"
                    parts.append(f"    #{count} {s['side_rule']} | {s['exit_rule']} | {s['entry_time_rule']} | "
                                 f"≤{s['entry_price_max']}¢ | n={s['sample_size']} | {wr} | {ev} | CI {ci} | ${pnl}")

            # Per-hour best (just top 1 per hour, compact)
            hour_strats = c.execute("""
                SELECT setup_key, strategy_key, side_rule, exit_rule, entry_time_rule,
                       entry_price_max, sample_size, win_rate, ev_per_trade_c, ci_lower
                FROM strategy_results
                WHERE setup_type = 'hour' AND sample_size >= 10
                ORDER BY setup_key, ev_per_trade_c DESC
            """).fetchall()
            hour_strats = rows_to_list(hour_strats)

            if hour_strats:
                parts.append(f"\n## Best Strategy Per Hour (top 1, n≥10)")
                seen_hours = set()
                for s in hour_strats:
                    if s['setup_key'] in seen_hours:
                        continue
                    seen_hours.add(s['setup_key'])
                    hour = s['setup_key'].replace('hour:', '') + ':00 ET'
                    wr = f"{(s['win_rate'] or 0)*100:.0f}%"
                    ev = f"{(s['ev_per_trade_c'] or 0):+.1f}¢"
                    parts.append(f"  {hour}: {s['side_rule']} | {s['exit_rule']} | {s['entry_time_rule']} | "
                                 f"≤{s['entry_price_max']}¢ | n={s['sample_size']} | {wr} | {ev}")
...
def api_chat():
    ...
    if not api_key:
        return jsonify({"error": "No API key set — add one in Settings → Services"}), 500
    ...
    # Support multi-round: use messages array if provided, else single message
    if not messages:
    ...
    system = (
        ...
        "TERMINOLOGY:\n"
        "Risk levels are: low, moderate, high, extreme, unknown. "
        "The internal DB stores 'terrible' but ALWAYS say 'extreme risk' to the user, never 'terrible'. "
        "Replace 'terrible' with 'extreme' in all output.\n\n"

        "CURRENT PHASE:\n"
        "The bot is in Phase 1 — data collection via shadow trading. It records every market's full "
        "price path, regime context, and outcome. In shadow mode, it also places 1-contract trades "
        "per market to measure real execution quality. Strategy simulation runs automatically every "
        "30 minutes against all recorded observations. The goal is to reach ~3,250 observations for "
        "FDR significance before trusting per-regime strategy recommendations. Most trades in the data "
        "are 'observed' (skipped) or 1-contract shadow trades, not full-size real trades.\n\n"

        "STRATEGY CONTEXT:\n"
        "This bot trades Kalshi BTC 15-minute binary options. "
        "Each strategy has four parameters: side rule, entry timing, max entry price, and sell target. "
        "Side rules (4): cheaper (buy whichever side costs less), "
        "yes (always YES), no (always NO), model (BTC fair value model picks side). "
        "The 'favored' side rule was removed — it had -8.2¢ global EV. "
        "Timing: early (enter ASAP), mid (wait 5 min), late (wait 10 min). "
        "Entry max: highest price willing to pay (5-95¢ in 5¢ steps). "
        "Sell target: absolute cents (e.g. sell@90¢) or hold to expiry. "
        "Strategy key format: side:timing:entry_max:sell_target (e.g. cheaper:early:45:90).\n\n"

        "KEY PRINCIPLES:\n"
        "- Win rate matters but EV per trade is the single most important metric.\n"
        "- Sample size matters enormously. Require n≥30 and CI lower >50% for reliability.\n"
        "- Below n=20, flag any finding as preliminary/insufficient data.\n"
        "- FDR-significant strategies (marked with ✓) have survived multiple testing correction.\n"
        "- Walk-forward OOS EV that diverges sharply from in-sample EV suggests overfitting.\n\n"

        "REGIME SYSTEM:\n"
        "The bot classifies BTC market conditions into regimes using technical indicators "
        "(volatility 1-5, trend -3 to +3, volume 1-5). Each regime gets a composite label. "
        "Strategy evaluation uses COARSE regimes (~15 buckets, 66% persistence) — not fine-grained "
        "labels (~63 buckets, 13% persistence) which were too sparse to reach significance. "
        "Fine labels are still recorded passively in observations for future research. "
        "The advisor hierarchy is: coarse_regime → hour → global.\n"
        "Regime stats (risk levels) are computed from REAL TRADES ONLY — observed/skipped markets do not count. "
        "During data collection, all regimes will be 'unknown' risk. "
        "The Strategy Observatory provides strategy-level risk from simulated data instead.\n"
        "- Risk levels: low (>=55% WR), moderate (>=45%), high (>=35%), extreme (<35%), unknown (<10 real trades)\n"
        "- Per-regime granular filters: volatility range, blocked hours, blocked days, blocked sides, "
        "max spread, and stability ceiling — all configurable per regime\n\n"

        "STRATEGY OBSERVATORY:\n"
        "Records every market's full price path and simulates strategy variants against each. "
        "Results aggregated per setup: global, coarse_regime (~15 buckets), hour (24 buckets). "
        "Metrics: unweighted and time-weighted EV (14-day half-life), walk-forward OOS validation "
        "(5-fold rolling), FDR correction (Benjamini-Hochberg t-test), slippage sensitivity (+1¢/+2¢), "
        "quality-split EV (full vs degraded observations), breakeven fee rate. "
        "When asked about strategies, reference sample size, CI, profit factor, and FDR significance — not just raw EV.\n\n"

        "BTC FAIR VALUE MODEL:\n"
        "The primary edge signal. Empirical probability surface: given BTC's distance from market-open "
        "price at time T, what is P(YES wins)? Compares model fair value to Kalshi prices after fees "
        "to compute edge. Three fallback tiers: vol-conditioned surface → global surface → analytical "
        "Brownian bridge. Drives model-based side selection, edge-scaled bet sizing, and dynamic sell "
        "target adjustment. The 'model' side rule is the best global performer at +2.2¢ weighted EV.\n\n"

        "BET SIZING:\n"
        "Three modes: flat (fixed dollar), percent (% of bankroll), edge_scaled (base bet × FV model "
        "edge tier multiplier). Edge-scaled uses the BTC Fair Value Model's computed edge at entry time "
        "to scale position size. Kelly sizing was removed (deferred until validated per-regime win rates "
        "are available from the Observatory).\n\n"

        "ALWAYS CHECK DATA BEFORE MAKING RECOMMENDATIONS:\n"
        "The 'Regime Stats' section contains LIFETIME statistics from REAL TRADES ONLY. "
        "During data collection phase, most regimes will show 0 trades — this is normal. "
        "Use Strategy Observatory results (simulation data) for analysis instead. "
        "A strategy needs n≥30 and positive OOS EV to be considered reliable. "
        "Never recommend changing settings based on a handful of observations.\n\n"
    ...
    try:
        ...
        if resp.status_code != 200:
            try:
                ...
                err = err_obj.get("message", resp.text[:200])
            ...
            hint = ""
            if resp.status_code == 401 or err_type == "authentication_error":
                key_preview = api_key[:10] + "..." if len(api_key) > 10 else "(empty)"
                hint = f" (key starts with: {key_preview} — check ANTHROPIC_API_KEY in supervisor config)"
            return jsonify({"error": f"API {resp.status_code}: {err}{hint}"}), 500
        result = resp.json()
...
# ═══════════════════════════════════════════════════════════════
#  REPORTS — Text, Chart, PDF
# ═══════════════════════════════════════════════════════════════

def _report_dir():
    d = os.path.join(os.environ.get("BOT_DIR", "/opt/15-min-btc-bot"), "_reports")
...
def _report_data():
    """Gather data needed for reports."""
    state = get_bot_state()
    cfg = get_all_config()
    trades = get_recent_trades(50)
    regimes = get_all_regime_stats()
    ls = get_lifetime_stats()
    return {"state": state, "cfg": cfg, "trades": trades,
            "regimes": regimes, "lifetime": ls or {}, "rounds": []}


@app.route("/api/report/text", methods=["POST"])
@requires_auth
def api_report_text():
    """Generate AI text report optimized for copying/external analysis."""
    api_key = _get_anthropic_key()
    if not api_key:
        return jsonify({"error": "No API key set — add one in Settings → Services"}), 500
    ...
    context = _gather_chat_context()
    system = (
        "Generate a comprehensive trading bot performance report. "
        "Format it as a clean, structured text document suitable for pasting into another AI or spreadsheet. "
        "Use plain text, no markdown. Use clear section headers with === underlines.\n\n"
        "IMPORTANT: The bot is currently in Phase 1 (data collection / observe-only mode). "
        "Most trades will be 'observed' (skipped), not real trades. Regime stats from real trades "
        "will be sparse or empty — this is expected. Focus analysis on Observatory simulation data.\n\n"
        "Always say 'extreme' instead of 'terrible' for risk levels.\n\n"
        "Include these sections:\n"
        "1. EXECUTIVE SUMMARY — 2-3 sentences on data collection progress and any insights\n"
        "2. SESSION STATS — wins, losses, P&L, skips, win rate\n"
        "3. LIFETIME STATS — total trades, overall win rate, total P&L\n"
        "4. REGIME ANALYSIS — for each regime: name, risk level, "
        "win rate, CI, trade count, P&L, current action (from risk_level_actions/regime_overrides). "
        "Flag any regimes where the action might need changing.\n"
        "5. OBSERVATORY INSIGHTS — observation count, top strategies by EV, "
        "FDR-significant strategies, walk-forward OOS results, best strategies per regime\n"
        "6. STREAK ANALYSIS — win/loss streaks, loss stops\n"
        "7. RECENT TRADE LOG — last 20 trades in tabular format: "
        "#ID | outcome | side@price | P&L | type | regime | time\n"
        "8. RECOMMENDATIONS — 2-3 actionable suggestions based on the data\n\n"
        "Be data-driven. Cite specific numbers. Keep it under 2000 words.\n\n"
        f"BOT DATA:\n{context}"
    )
    ...
    try:
        resp = http_requests.post(
            ...
            json={"model": "claude-sonnet-4-6", "max_tokens": 4096,
                  "system": system,
                  "messages": [{"role": "user", "content": "Generate the full performance report now."}]},
            timeout=60,
        )
        if resp.status_code != 200:
        ...
        text = "".join(b.get("text", "") for b in result.get("content", []))
        usage = result.get("usage", {})
        return jsonify({"report": text, "usage": usage})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_report_chart():
    ...
    try:
        ...
        pip_check = ""
        try:
            r = subprocess.run([py, "-m", "pip", "show", "matplotlib"],
                             capture_output=True, text=True, timeout=10)
            pip_check = r.stdout.strip() or r.stderr.strip()
        except: pass
        return jsonify({"error": (
            f"matplotlib import failed: {exc}. "
            f"Python: {py} (v{sys.version.split()[0]}). "
            f"pip show: {pip_check[:200]}. "
            f"Fix: run in terminal: {py} -m pip install matplotlib --break-system-packages"
        )}), 500

    data = _report_data()
    trades = [t for t in data["trades"] if t.get("outcome") in ("win", "loss")][:30]
    trades.reverse()  # Oldest first
    regimes = sorted(data["regimes"], key=lambda r: r.get("total_trades", 0), reverse=True)[:10]
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 7), gridspec_kw={'height_ratios': [3, 2]})
    ...
    if trades:
        ...
        is_data = [bool(t.get('is_data_collection', 0)) for t in trades]
        colors = []
        for i, t in enumerate(trades):
            p = pnls[i]
            if is_data[i]:
                colors.append('#2d5a3d' if p >= 0 else '#5a2d2d')  # Dim green/red for data bets
            else:
                colors.append(green if p >= 0 else red)
        bars = ax1.bar(range(len(pnls)), pnls, color=colors, width=0.7, edgecolor='none')
        # Cumulative P&L line
        cum = []
        s = 0
        for p in pnls:
            s += p
            cum.append(s)
        ax1.plot(range(len(cum)), cum, color=blue, linewidth=2, alpha=0.8, zorder=5)
        ax1.axhline(y=0, color=border, linewidth=0.5)

        # Data bet markers
        for i, d in enumerate(is_data):
            if d:
                ax1.plot(i, pnls[i] + (0.3 if pnls[i] >= 0 else -0.3), 'D',
                        color=yellow, markersize=4, zorder=6)

    ax1.set_title('Recent Trade P&L', color=text_c, fontsize=13, fontweight='bold', pad=10)
    ax1.set_xlabel('Trade', color=dim, fontsize=9)
    ax1.set_ylabel('P&L ($)', color=dim, fontsize=9)
    ...
    legend_items = [Patch(facecolor=green, label='Win'),
                    Patch(facecolor=red, label='Loss'),
                    Patch(facecolor='#2d5a3d', label='Data bet'),
                    Line2D([0], [0], color=blue, lw=2, label='Cumulative')]
    ax1.legend(handles=legend_items, loc='upper left', fontsize=7,
              facecolor=card, edgecolor=border, labelcolor=dim)

    # ── Bottom: Regime win rates ──
    ax2.set_facecolor(card)
    if regimes:
        labels = [r['regime_label'].replace('_', ' ')[:20] for r in regimes]
        wrs = [(r.get('win_rate', 0) * 100) for r in regimes]
        risk_colors = {'low': green, 'moderate': yellow, 'high': orange, 'terrible': red, 'unknown': dim}
        colors2 = [risk_colors.get(r.get('risk_level', ''), dim) for r in regimes]
        counts = [str(r.get('total_trades', 0)) for r in regimes]

        y_pos = range(len(labels))
        ax2.barh(y_pos, wrs, color=colors2, height=0.6, edgecolor='none')
        ax2.set_yticks(y_pos)
        ax2.set_yticklabels(labels, fontsize=7, color=text_c)
        ax2.invert_yaxis()
        ax2.axvline(x=50, color=border, linewidth=0.5, linestyle='--')
        for i, (w, c) in enumerate(zip(wrs, counts)):
            ax2.text(w + 1, i, f'{w:.0f}% ({c})', va='center', fontsize=7, color=dim)

    ax2.set_title('Regime Win Rates (top 10 by trade count)', color=text_c, fontsize=13,
                  fontweight='bold', pad=10)
    ax2.set_xlabel('Win Rate %', color=dim, fontsize=9)
    ...
    # Session info footer
    state = data["state"]
    sw = state.get('session_wins', 0)
    sl = state.get('session_losses', 0)
    spnl = state.get('session_pnl', 0)
    fig.text(0.5, 0.01,
             f"Session: {sw}W/{sl}L | P&L: {fpnl(spnl)} | "
             f"Bankroll: ${(state.get('bankroll_cents',0)/100):.2f}",
             ha='center', fontsize=9, color=dim)

    plt.tight_layout(rect=[0, 0.03, 1, 1])

    fname = f"chart_{int(datetime.now(timezone.utc).timestamp())}.png"
    fpath = os.path.join(_report_dir(), fname)
    fig.savefig(fpath, dpi=150, facecolor=bg, edgecolor='none', bbox_inches='tight')
    plt.close(fig)

    return jsonify({"url": f"/reports/{fname}"})
...
def api_report_pdf():
    ...
    trades = [t for t in data["trades"] if t.get("outcome") in ("win", "loss")][:30]
    trades.reverse()
    regimes = sorted(data["regimes"], key=lambda r: r.get("total_trades", 0), reverse=True)[:12]
    state = data["state"]
    ls = data["lifetime"]
    ...
    fname = f"report_{int(datetime.now(timezone.utc).timestamp())}.pdf"
    fpath = os.path.join(_report_dir(), fname)
    ...
    with PdfPages(fpath) as pdf:
        ...
        lines = []
        lines.append("BOT PERFORMANCE REPORT")
        lines.append(f"Generated: {datetime.now(ZoneInfo('America/Chicago')).strftime('%Y-%m-%d %I:%M %p CT')}")
        lines.append("")
        lines.append("SESSION STATS")
        sw, sl = state.get('session_wins', 0), state.get('session_losses', 0)
        st = sw + sl
        wr = (sw / st * 100) if st > 0 else 0
        spnl = state.get('session_pnl', 0)
        lines.append(f"  Trades: {st}  ({sw}W / {sl}L)  Win Rate: {wr:.1f}%")
        lines.append(f"  P&L: {fpnl(spnl)}  Observed: {state.get('session_skips', 0)}")
        lines.append(f"  Bankroll: ${state.get('bankroll_cents', 0)/100:.2f}")
        lines.append("")
        lines.append("LIFETIME STATS")
        lw = ls.get('total_wins', state.get('lifetime_wins', 0))
        ll = ls.get('total_losses', state.get('lifetime_losses', 0))
        lt = lw + ll
        lwr = (lw / lt * 100) if lt > 0 else 0
        lpnl = state.get('lifetime_pnl', 0)
        lines.append(f"  Trades: {lt}  ({lw}W / {ll}L)  Win Rate: {lwr:.1f}%")
        lines.append(f"  Total P&L: {fpnl(lpnl)}")
        if ls.get('max_win_streak'):
            lines.append(f"  Best Streak: {ls['max_win_streak']}W  Worst: {ls.get('max_loss_streak', 0)}L")
        lines.append("")
        lines.append("REGIME OVERVIEW")
        _rd = lambda r: 'extreme' if r == 'terrible' else r
        for r in regimes:
            rl = _rd(r.get('risk_level', '?'))
            rwr = (r.get('win_rate', 0) * 100)
            lines.append(f"  {r['regime_label']}: {rl} risk, {rwr:.0f}% WR, "
                        f"{r.get('total_trades', 0)} trades, {fpnl(r.get('total_pnl', 0))}")
        lines.append("")

        ax.text(0.05, 0.95, "\n".join(lines), transform=ax.transAxes, fontsize=9.5,
                verticalalignment='top', fontfamily='monospace', color=text_c,
                linespacing=1.4)
        pdf.savefig(fig1, facecolor=bg)
        plt.close(fig1)

        # ── Page 2: Trade P&L Chart ──
        fig2, ax2 = plt.subplots(figsize=(8.5, 5.5))
        fig2.patch.set_facecolor(bg)
        ax2.set_facecolor(card)
        if trades:
            pnls = [t.get('pnl', 0) for t in trades]
            is_data_list = [bool(t.get('is_data_collection', 0)) for t in trades]
            colors = []
            for i, t in enumerate(trades):
                p = pnls[i]
                if is_data_list[i]:
                    colors.append('#2d5a3d' if p >= 0 else '#5a2d2d')
                else:
                    colors.append(green if p >= 0 else red)
            ...
            ax2.plot(range(len(cum)), cum, color=blue, linewidth=2, alpha=0.8)
            ax2.axhline(y=0, color=border, linewidth=0.5)
            for i, d in enumerate(is_data_list):
                if d:
                    ax2.plot(i, pnls[i] + (0.2 if pnls[i] >= 0 else -0.2), 'D',
                            color=yellow, markersize=3)
        ax2.set_title('Recent Trade P&L', color=text_c, fontsize=13, fontweight='bold')
        ax2.set_xlabel('Trade', color=dim, fontsize=9)
        ax2.set_ylabel('P&L ($)', color=dim, fontsize=9)
        ax2.tick_params(colors=dim, labelsize=8)
        ...
        # ── Page 3: Regime Win Rates ──
        fig3, ax3 = plt.subplots(figsize=(8.5, 5.5))
        fig3.patch.set_facecolor(bg)
        ax3.set_facecolor(card)
        if regimes:
            labels = [r['regime_label'].replace('_', ' ')[:25] for r in regimes]
            wrs = [(r.get('win_rate', 0) * 100) for r in regimes]
            risk_colors = {'low': green, 'moderate': yellow, 'high': orange, 'terrible': red, 'unknown': dim}
            cs = [risk_colors.get(r.get('risk_level', ''), dim) for r in regimes]
            y_pos = range(len(labels))
            ax3.barh(y_pos, wrs, color=cs, height=0.6)
            ax3.set_yticks(y_pos)
            ax3.set_yticklabels(labels, fontsize=7, color=text_c)
            ax3.invert_yaxis()
            ax3.axvline(x=50, color=border, linewidth=0.5, linestyle='--')
            for i, (w, r) in enumerate(zip(wrs, regimes)):
                ax3.text(w + 1, i, f'{w:.0f}% ({r.get("total_trades", 0)})',
                        va='center', fontsize=7, color=dim)
        ax3.set_title('Regime Win Rates', color=text_c, fontsize=13, fontweight='bold')
        ax3.set_xlabel('Win Rate %', color=dim, fontsize=9)
    ...
    try:
        ...
        with get_conn() as c:
            ...
            strat_meta = c.execute("""
                ...
                FROM strategy_results WHERE sample_size >= 10
            """).fetchone()
...
def api_deploy_restart():
    ...
    results = {}
    bot_services = [s for s in services if s != "kalshi-dashboard"]
    ...
    if "kalshi-dashboard" in services:
        results["kalshi-dashboard"] = "restarting in 1s..."
        def _delayed_restart():
            import time as _t
            _t.sleep(1)
            try:
                subprocess.run(["supervisorctl", "restart", "kalshi-dashboard"],
                               capture_output=True, text=True, timeout=10)
            except Exception:
                pass
...
def index():
    html = MAIN_HTML
    try:
        import importlib, game as _gm
        importlib.reload(_gm)
        html = html.replace("<!-- ARCADE_CONTENT -->", _gm.GAME_HTML)
        html = html.replace("// ARCADE_JS_PLACEHOLDER", _gm.GAME_JS)
    except Exception as e:
        html = html.replace("<!-- ARCADE_CONTENT -->",
            '<div class="dim" style="padding:20px;text-align:center">Game: ' + str(e) + '</div>')
        html = html.replace("// ARCADE_JS_PLACEHOLDER", "")
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .chat-copy { background: none; border: none; padding: 4px 0; font-size: 11px; color: var(--dim);
    cursor: pointer; -webkit-tap-highlight-color: transparent; display: block;
    margin-top: 4px; opacity: 0.4; transition: opacity 0.15s; width: 20px; height: 20px; }
  .chat-copy:hover, .chat-copy:active { opacity: 0.8; }
  .chat-copy svg { width: 14px; height: 14px; }
  .chat-user .chat-copy { margin-left: auto; }
  .chat-ai .chat-copy { margin-left: 0; }
  .chat-user { text-align: right; }
  .chat-user > div { display:inline-block; background:rgba(88,166,255,0.08); border:1px solid rgba(88,166,255,0.3);
  ...
  .chat-ai > div { display:inline-block; background:rgba(255,255,255,0.02); border:1px solid var(--border);
  ...
  .chat-thinking > div { display:inline-block; background:rgba(255,255,255,0.02); border:1px solid var(--border);
  ...
  .ctrl-icon { width: 48px; height: 48px; border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    margin: 2px auto 0; transition: all 0.2s ease; }
  .ctrl-icon svg { width: 24px; height: 24px; transition: fill 0.2s, filter 0.2s; }
  .ctrl-play { background: var(--green);
    box-shadow: 0 0 16px rgba(63,185,80,0.35); }
  .ctrl-play svg { fill: #000; }
  .ctrl-play:disabled svg { fill: var(--dim); }
  .ctrl-stop { background: var(--red);
    box-shadow: 0 0 16px rgba(248,81,73,0.35); }
  .ctrl-stop svg { fill: #000; }
  .ctrl-stop:disabled svg { fill: var(--dim); }
  .ctrl-stop.stop-pending { background: var(--dim);
    box-shadow: none; }
  .ctrl-stop.stop-pending svg { fill: #000; }
  .ctrl-observe { background: var(--yellow);
    box-shadow: 0 0 16px rgba(227,179,65,0.35); }
  .ctrl-observe svg { fill: #000; }
  ...
  .chat-action-bar { display: flex; gap: 8px; flex-wrap: wrap; margin-top: 4px; }
  .chat-action-btn { display: inline-flex; align-items: center; gap: 5px;
  ...
  .chat-action-btn:active { border-color: var(--blue); background: rgba(88,166,255,0.08); }
  .chat-action-btn svg { width: 16px; height: 16px; flex-shrink: 0; }
  .chat-report-card { background: var(--bg); border: 1px solid var(--border); border-radius: 8px;
    padding: 12px; margin-top: 6px; max-height: 300px; overflow-y: auto; }
  .chat-report-card pre { white-space: pre-wrap; font-size: 11px; font-family: monospace;
  ...
  /* Chat overlay */
  #chatOverlay { position: fixed; left: 0; right: 0;
    z-index: 95; display: none; overflow: hidden; background: var(--bg); }
  #chatOverlay .chat-inner { display: flex; flex-direction: column;
    width: 100%; max-width: 600px; margin: 0 auto; height: 100%; overflow: hidden; }
  #chatOverlay .chat-hdr { flex-shrink: 0; }
  #chatOverlay #chatMessages { flex: 1; overflow-y: auto; overscroll-behavior: contain; }
  #chatOverlay .chat-input { flex-shrink: 0; }
</style>
</head>
<body>
    ...
    <div style="border-top:1px solid var(--border);padding-top:8px">
      ...
      <div class="detail-section" id="bkmLockSection">
        ...
        <div class="input-row" style="margin-top:8px">
          ...
          <button class="act-btn act-btn-blue act-btn-sm"
...
<div style="padding:0 16px;margin-top:12px" id="sessionStatsSection">
  ...
  <div class="stat-grid">
    ...
    <div class="stat"><div class="label">Peak P&L</div><div class="val" id="statPeakPnl">—</div></div>
    <div class="stat"><div class="label">Drawdown</div><div class="val" id="statDrawdown">—</div></div>
  </div>
  <div class="stat-grid">
    ...
    <div class="stat"><div class="label">Avg P&L</div><div class="val" id="statAvgPnl">—</div></div>
  </div>
...
<div id="pageSettings" class="page" style="padding:0 12px">
  ...
  <div class="settings-card">
    ...
    <div class="toggle" style="margin-top:0">
      <label class="tog"><input type="checkbox" id="ignoreMode"
             onchange="saveSetting('ignore_mode',this.checked)"><span class="tpill"></span></label>
      <span class="dim" style="color:var(--orange)">Ignore Mode — trades won't count in stats</span>
    </div>
  ...
  <!-- ─── MONEY MANAGEMENT ─── -->
  <div class="settings-card">
    <div class="sc-title">MONEY MANAGEMENT</div>
    ...
    <div class="input-row" style="margin-top:0">
      ...
      <span class="dim">$ (stops trading)</span>
    </div>
    <div class="input-row">
    ...
    <div class="input-row">
      ...
      <span class="dim">$ profit to stop</span>
    </div>
    <div class="input-row">
      <label>Session Loss Limit</label>
      <input type="number" id="sessionLossLimit" min="0" step="10"
             onchange="saveSetting('session_loss_limit',parseFloat(this.value)||0)">
      <span class="dim">$ loss to stop (0=off)</span>
    </div>

    <div class="sc-sub">CIRCUIT BREAKERS</div>
    <div class="input-row" style="margin-top:0">
      <label>Rolling WR Window</label>
      <input type="number" id="rollingWrWindow" min="0" max="100" step="5"
             style="width:60px" onchange="saveSetting('rolling_wr_window',parseInt(this.value)||0)">
      <span class="dim">last N trades (0=off)</span>
    </div>
    <div class="input-row">
      <label>Rolling WR Floor</label>
      <input type="number" id="rollingWrFloor" min="0" max="100" step="5"
             style="width:60px" onchange="saveSetting('rolling_wr_floor',parseFloat(this.value)||0)">
      <span class="dim">% min win rate</span>
    </div>

    <div class="sc-sub">AUTO-LOCK PROFITS</div>
    <div class="toggle" style="margin-top:0">
      <label class="tog"><input type="checkbox" id="autoLockEnabled"
             onchange="saveSetting('auto_lock_enabled',this.checked)"><span class="tpill"></span></label>
      <span class="dim">Enable auto-lock</span>
    </div>
    <div class="input-row">
    ...
    <div class="toggle">
      <label class="tog"><input type="checkbox" id="autoLockRandom"
             onchange="saveSetting('auto_lock_random',this.checked)"><span class="tpill"></span></label>
      <span class="dim">50/50 lock or grow bankroll</span>
    </div>
    <div class="input-row" style="margin-top:8px">
      <label>Profit Goal</label>
      <input type="number" id="profitGoal" min="0" step="100"
             onchange="saveSetting('profit_goal',parseFloat(this.value)||0);saveSetting('profit_goal_reached',false)">
      <span class="dim">$ (0 = off)</span>
    </div>
    <div class="sc-hint">Notifies when locked profits reach this amount.</div>
  </div>
  ...
  <div class="settings-card">
    ...
    <div class="toggle"><label class="tog"><input type="checkbox" id="notifyAutoLock" onchange="saveSetting('push_notify_auto_lock',this.checked)" checked><span class="tpill"></span></label><span class="dim">Auto-lock</span></div>
  ...
  <div class="settings-card">
    ...
    <div id="svcStatus" style="font-size:11px;margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid var(--border)">
        ...
        <div style="display:flex;gap:4px">
          <button class="btn btn-dim" style="font-size:10px;padding:3px 8px" onclick="svcControl('start','kalshi-bot')">Start</button>
          <button class="btn btn-dim" style="font-size:10px;padding:3px 8px" onclick="svcControl('stop','kalshi-bot')">Stop</button>
          <button class="btn btn-dim" style="font-size:10px;padding:3px 8px" onclick="svcControl('restart','kalshi-bot')">Restart</button>
        </div>
      </div>
      <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0">
        <div><strong>Dashboard</strong> <span id="svcDashStatus" class="dim">—</span></div>
        <div style="display:flex;gap:4px">
          <button class="btn btn-dim" style="font-size:10px;padding:3px 8px" onclick="svcControl('start','kalshi-dashboard')">Start</button>
          <button class="btn btn-dim" style="font-size:10px;padding:3px 8px" onclick="svcControl('stop','kalshi-dashboard')">Stop</button>
          <button class="btn btn-dim" style="font-size:10px;padding:3px 8px" onclick="svcControl('restart','kalshi-dashboard')">Restart</button>
        </div>
      </div>
    </div>
    ...
    <div class="sc-sub" style="margin-top:12px">AI API KEY</div>
    <div id="apiKeyStatus" class="dim" style="font-size:11px;margin-bottom:6px">Checking...</div>
    <div style="display:flex;gap:6px;align-items:center">
      <input type="password" id="apiKeyInput" placeholder="sk-ant-api03-..."
             style="flex:1;font-size:12px;font-family:monospace;padding:8px;background:var(--bg);border:1px solid var(--border);color:var(--text);border-radius:6px">
      <button class="btn btn-blue" style="font-size:11px;padding:8px 12px;white-space:nowrap" onclick="_saveApiKey()">Save</button>
    </div>
    <div class="sc-hint" style="margin-top:4px">Stored securely in bot config. Overrides .env key.</div>
  </div>
  ...
  <div class="settings-card" style="border-color:rgba(248,81,73,0.2)">
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      <button class="btn btn-dim" style="text-align:left;font-size:12px;padding:10px" onclick="_resetConfirm1('session','Reset Session','Clears session stats and cooldown. Regime data and trade history preserved.')">
        Reset Session
        <div class="dim" style="font-size:10px;margin-top:2px">Stats, cooldown → zero</div>
      </button>
      <button class="btn btn-dim" style="text-align:left;font-size:12px;padding:10px" onclick="_resetConfirm1('locked','Zero Locked Bankroll','Unlocks all locked profits. Trade history preserved.')">
        Zero Locked Bankroll
        <div class="dim" style="font-size:10px;margin-top:2px">Unlock all profits</div>
      </button>
      <button class="btn btn-dim" style="text-align:left;font-size:12px;padding:10px" onclick="_resetConfirm1('settings','Reset Settings','Restores all settings to defaults. Locked bankroll and credentials preserved.')">
        Reset Settings
        <div class="dim" style="font-size:10px;margin-top:2px">All config → defaults (keeps locked $)</div>
      </button>
    ...
    <div style="border-top:1px solid var(--border);margin-top:10px;padding-top:10px">
      <button class="btn btn-dim" style="text-align:left;font-size:12px;padding:10px;width:100%;border-color:rgba(210,153,34,0.3)" onclick="_resetConfirm1('soft','Soft Reset','Resets session, locked bankroll, and cooldown. Keeps all trade history and regime data.')">
        <span style="color:var(--yellow)">Soft Reset</span>
        <div class="dim" style="font-size:10px;margin-top:2px">Session + locked → zero (keeps trades &amp; regimes)</div>
      </button>
    </div>
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;padding:0 4px;margin-bottom:12px">
    ...
    <span onclick="switchTab('Arcade')" style="cursor:pointer;opacity:0.15;-webkit-tap-highlight-color:transparent">
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="12" rx="2"/><line x1="6" y1="10" x2="6" y2="14"/><line x1="4" y1="12" x2="8" y2="12"/><circle cx="16" cy="10" r="1" fill="currentColor"/><circle cx="19" cy="12" r="1" fill="currentColor"/></svg>
    </span>
...
<!-- ═══ PAGE: BITCOIN ═══ -->
<div id="pageBitcoin" class="page" style="padding:0 16px">
...
<div class="confirm-overlay" id="cashOutOverlay">
  <div class="confirm-box">
    ...
    <div id="cashOutConfirmEstimate" style="margin:10px 0;padding:10px;background:var(--bg);border-radius:8px;font-family:monospace;text-align:center;font-size:13px"></div>
    <p>This will aggressively sell all contracts at the best available price.</p>
    <p class="dim" style="margin-top:8px">This trade is tracked separately (ignored in stats).</p>
...
<!-- Reset Session -->
<div class="confirm-overlay" id="resetSessionOverlay" style="display:none">
  <div class="confirm-box" style="border-color:var(--orange);max-width:300px">
    <h3 style="color:var(--orange)">Reset Session?</h3>
    <div style="font-size:13px;color:var(--dim);margin:10px 0">All session stats (wins, losses, P&L, skips) will be zeroed out.</div>
    <div class="confirm-btns">
      <button class="btn btn-dim" onclick="closeModal('resetSessionOverlay')">Cancel</button>
      <button class="btn btn-red" onclick="doResetSession()" style="border-color:var(--orange);background:var(--orange);color:#000">Reset</button>
    </div>
  </div>
</div>
...
<!-- Recover Session Confirmation -->
<div class="confirm-overlay" id="recoverSessionOverlay" style="display:none">
  <div class="confirm-box" style="border-color:var(--blue);max-width:300px">
    <h3 style="color:var(--blue)">Recover Session?</h3>
    <div id="recoverSessionDesc" style="margin:10px 0;padding:10px;background:var(--bg);border-radius:8px;font-family:monospace;text-align:center;font-size:14px;font-weight:600"></div>
    <div style="font-size:12px;color:var(--dim);margin-bottom:12px">This merges the previous session stats into your current session.</div>
    <div class="confirm-btns">
      <button class="btn btn-dim" onclick="closeModal('recoverSessionOverlay')">Cancel</button>
      <button class="btn btn-blue" onclick="doRecoverSession()">Recover</button>
    </div>
  </div>
</div>
...
<!-- AI Chat -->
<!-- ═══ PAGE: CHAT ═══ -->
<div id="pageChat" class="page" style="padding:0">
  <!-- This page is empty - chat UI lives in #chatOverlay -->
</div>

<!-- ═══ PAGE: ARCADE (secret) ═══ -->
<div id="pageArcade" class="page" style="padding:8px 12px">
<!-- ARCADE_CONTENT -->
</div>

</div> <!-- end contentWrap -->

<!-- Chat overlay — full screen flex column, touchmove locked -->
<div id="chatOverlay">
  <div class="chat-inner">
    <!-- Chat sub-header -->
    <div class="chat-hdr" style="padding:6px 16px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--border)">
      <div style="display:flex;align-items:center;gap:8px">
        <span style="font-size:13px;font-weight:600;color:var(--blue)">Ask AI</span>
        <span id="chatUsageInfo" class="dim" style="font-size:10px;font-family:monospace"></span>
      </div>
      <div style="display:flex;gap:6px">
        <button onclick="exportAIData(this)" style="background:none;border:1px solid var(--border);border-radius:4px;color:var(--dim);cursor:pointer;padding:2px 8px;font-size:10px;-webkit-tap-highlight-color:transparent">Export Data</button>
        <button onclick="copyEntireChat()" style="background:none;border:1px solid var(--border);border-radius:4px;color:var(--dim);cursor:pointer;padding:2px 8px;font-size:10px;-webkit-tap-highlight-color:transparent">Copy All</button>
        <button onclick="clearChat()" style="background:none;border:1px solid var(--border);border-radius:4px;color:var(--dim);cursor:pointer;padding:2px 8px;font-size:10px;-webkit-tap-highlight-color:transparent">Clear</button>
      </div>
    </div>
    <!-- Messages -->
    <div id="chatMessages" style="padding:16px" onclick="if(event.target===this||!event.target.closest('button,a,input')){$('#chatInput').blur()}">
      <div id="chatWelcome" style="text-align:center;padding:30px 10px">
        ...
        <div id="chatSuggestions" style="display:flex;flex-wrap:wrap;gap:6px;justify-content:center;margin-top:16px">
          <button class="chat-chip" onclick="askSuggestion(this)">Summarize current session</button>
          ...
          <button class="chat-chip" onclick="triggerReport()">Generate a report</button>
        </div>
      </div>
    </div>
    <!-- Input -->
    <div class="chat-input" style="padding:10px 16px 10px;background:var(--card);border-top:1px solid var(--border)">
      <div style="display:flex;gap:8px;align-items:flex-end">
        <input type="text" id="chatInput" placeholder="Ask a question..." autocapitalize="sentences" autocomplete="off" style="flex:1;background:var(--bg);border:1px solid var(--border);border-radius:20px;padding:10px 16px;color:var(--text);font-size:16px;outline:none" onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();sendChat()}">
        <button onclick="sendChat()" id="chatSendBtn" style="background:rgba(88,166,255,0.1);border:1px solid rgba(88,166,255,0.3);border-radius:50%;width:38px;height:38px;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;-webkit-tap-highlight-color:transparent">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--blue)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 2L11 13"/><path d="M22 2L15 22L11 13L2 9L22 2Z"/></svg>
        </button>
      </div>
      <div id="chatUsageBar" style="display:none;margin-top:6px;padding:0 4px;font-size:10px;font-family:monospace;color:var(--dim);text-align:center"></div>
    </div>
  </div>
</div>
...
<div class="tab-bar">
  ...
  <button class="tab-btn" data-tab="Bitcoin" onclick="switchTab('Bitcoin')">
    <svg viewBox="0 0 64 64" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><circle cx="32" cy="32" r="28"/><path d="M25 44V20h8.5c5 0 8 2.5 8 6.5 0 3-2 5-4.5 5.5 3 .5 5.5 3 5.5 6.5 0 4.5-3.5 5.5-8.5 5.5H25z"/><path d="M29 31h4.5c2.5 0 4-1.2 4-3.5s-1.5-3.5-4-3.5H29v7zm0 9h5.5c3 0 4.5-1.2 4.5-3.5S37.5 33 34.5 33H29v7z"/><line x1="28" y1="17" x2="28" y2="20"/><line x1="34" y1="17" x2="34" y2="20"/><line x1="28" y1="44" x2="28" y2="47"/><line x1="34" y1="44" x2="34" y2="47"/></svg>
    <span>Bitcoin</span>
  </button>
  <button class="tab-btn tab-btn-main" onclick="toggleBot()">
  ...
  <button class="tab-btn" data-tab="Chat" onclick="switchTab('Chat')">
    <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>
    <span>AI</span>
  </button>
...
function _autoFitStatVals() {
  document.querySelectorAll('.stat-grid .stat .val').forEach(el => {
    el.style.fontSize = '';
    let size = 18;
...
(function() {
  ...
  document.addEventListener('touchstart', e => {
    ...
    if (atTop && e.touches.length === 1 && !isModalOpen() && !_chartTouchActive && _currentTab !== 'Chat') {
...
// Client-side session stat tracking
let _sessTrack = {
  prevWins: -1, prevLosses: -1,
  curStreakType: null, curStreakLen: 0,
...
function openBankrollModal() {
  ...
  // Trading info
  const lossStreak = s.loss_streak || 0;
...
async function toggleBot() {
  const s = _uiState;
  const isRunning = !!s.auto_trading;
  const btn = $('#btnMain');
  if (isRunning) {
    // Running → Stop
    const at = s.active_trade;
    if (at) {
      ...
      patchUI({auto_trading: 0, status: 'stopped', status_detail: 'Stopped', loss_streak: 0});
      cmd('stop');
      showToast('Stopping...', 'red');
    }
  } else {
    // Stopped → Start in current mode
    cmd('start', {mode: 'continuous', count: 0});
    btn.className = 'ctrl-icon ctrl-play';
    btn.querySelector('svg').innerHTML = `<path d="${SVG_PATH_PLAY}"/>`;
    showToast('Starting...', 'green');
  }
}
...
async function svcControl(action, service) {
  ...
  try {
    ...
    if (service === 'kalshi-dashboard' || service === 'all') {
...
async function loadSvcStatus() {
  try {
    ...
    if (r['kalshi-dashboard']) {
      const s = r['kalshi-dashboard'];
      const color = s.status === 'RUNNING' ? 'var(--green)' : 'var(--red)';
...
// ── Controls ─────────────────────────────────────────────
const SVG_PATH_PLAY = 'M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.347a1.125 1.125 0 0 1 0 1.972l-11.54 6.347a1.125 1.125 0 0 1-1.667-.986V5.653Z';
const SVG_PATH_STOP = 'M5.25 7.5A2.25 2.25 0 0 1 7.5 5.25h9a2.25 2.25 0 0 1 2.25 2.25v9a2.25 2.25 0 0 1-2.25 2.25h-9a2.25 2.25 0 0 1-2.25-2.25v-9Z';
const SVG_PATH_OBSERVE = 'M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z';
...
function showCashOut() {
  // Populate estimate in confirmation dialog
  const est = document.getElementById('cashOutConfirmEstimate');
  const at = _uiState && _uiState.active_trade;
  if (est && at) {
    const coBid = at.current_bid != null ? at.current_bid : 0;
    const coCost = at.actual_cost || 0;
    const coShares = at.fill_count || 0;
    const coSold = at.sell_progress || 0;
    const coRemaining = coShares - coSold;
    const coGross = coRemaining * coBid / 100;
    const coAlreadySold = coSold * (at.sell_price_c || 0) / 100;
    const coTotal = coGross + coAlreadySold;
    const coPnl = coTotal - coCost;
    if (coBid > 0 && coRemaining > 0) {
      const pnlSign = coPnl >= 0 ? '+' : '';
      const pnlColor = coPnl >= 0 ? 'var(--green)' : 'var(--red)';
      est.innerHTML = `<div style="margin-bottom:4px">Sell ${coRemaining} contract${coRemaining>1?'s':''} @ ${coBid}¢</div>`
        + `<div>Est. proceeds: <strong>$${coTotal.toFixed(2)}</strong> · Cost: $${coCost.toFixed(2)}</div>`
        + `<div style="font-size:16px;margin-top:4px;color:${pnlColor}"><strong>${fmtPnl(coPnl)}</strong></div>`;
      est.style.display = '';
    } else {
      est.style.display = 'none';
    }
  }
  openModal('cashOutOverlay');
}
...
async function confirmCashOut() {
  ...
  showToast('Selling all contracts...', 'red');
}
...
function resetSession() {
  openModal('resetSessionOverlay');
}

function doResetSession() {
  closeModal('resetSessionOverlay');
  lastStateData._lastTrade = null;
  _sessTrack = {
    prevWins: 0, prevLosses: 0,
    curStreakType: null, curStreakLen: 0,
    maxWinStreak: 0, maxLossStreak: 0,
    peakPnl: 0,
  ...
  api('/api/command', {method:'POST', headers:{'Content-Type':'application/json','X-CSRF-Token':_getCsrfToken()},
    body: JSON.stringify({command:'reset_session', params:{}})});
  showToast('Session reset', 'blue');
...
function recoverSession() {
  const prev = _uiState._prev_session;
  if (!prev) { showToast('No previous session to recover', 'yellow'); return; }
  let parsed;
  try { parsed = typeof prev === 'string' ? JSON.parse(prev) : prev; } catch(e) { return; }
  const w = parsed.wins || 0, l = parsed.losses || 0, p = parsed.pnl || 0;
  const pColor = p >= 0 ? 'var(--green)' : 'var(--red)';
  $('#recoverSessionDesc').innerHTML = `${w}W – ${l}L · <span style="color:${pColor}">${fmtPnl(p)}</span>`;
  openModal('recoverSessionOverlay');
}

function doRecoverSession() {
  closeModal('recoverSessionOverlay');
  const prev = _uiState._prev_session;
  if (!prev) return;
  let parsed;
  try { parsed = typeof prev === 'string' ? JSON.parse(prev) : prev; } catch(e) { return; }
  const desc = `${parsed.wins||0}W-${parsed.losses||0}L, ${fmtPnl(parsed.pnl||0)}`;
  api('/api/command', {method:'POST', headers:{'Content-Type':'application/json','X-CSRF-Token':_getCsrfToken()},
    body: JSON.stringify({command:'recover_session', params:{}})});
  showToast('Session recovered: ' + desc, 'green');
  setTimeout(pollState, 500);
}
...
let _uiState = {};
let _autoTradingSince = null;
try { const _saved = sessionStorage.getItem('_autoSince'); if (_saved) _autoTradingSince = parseInt(_saved); } catch(e) {}
...
function renderUI(s) {
  try {
    ...
    // Center button: simple Start/Stop
    const btn = $('#btnMain');
    if (isRunning) {
      btn.className = 'ctrl-icon ctrl-stop';
      btn.querySelector('svg').innerHTML = `<path d="${SVG_PATH_STOP}"/>`;
    } else {
      btn.className = 'ctrl-icon ctrl-play';
      btn.querySelector('svg').innerHTML = `<path d="${SVG_PATH_PLAY}"/>`;
    }
    ...
    if (_botStale) {
      // ── BOT OFFLINE — override status display ──
      statusMain = 'Bot Offline';
      statusColor = 'var(--red)';
      dotClass = 'dot-red';
      subParts.push('Process may have crashed — check server');
      ...
      if (tradingMode === 'observe' || tradingMode === 'shadow' || tradingMode === 'hybrid') {
        ...
        subParts.push('Observatory recording all markets');
      } else {
        statusMain = 'Stopped';
        if (hasActiveTrade) statusMain = 'Stopped · trade active';

        // Bet config (only show when live trading mode)
        const betMode = $('#betMode')?.value || 'flat';
        const betSize = parseFloat($('#betSize')?.value) || 0;
        let betLabel = betMode === 'percent' ? `${betSize}% ($${((betSize/100)*currentBankroll).toFixed(0)})` :
                      `$${betSize.toFixed(0)}`;
        subParts.push(`Bet: ${betLabel}`);
        const stratSell = $('#strategySell')?.value || 'hold';
        const stratSellLabel = stratSell === 'hold' ? 'Hold' : 'sell@' + stratSell + '¢';
        subParts[subParts.length-1] += ` · ${stratSellLabel}`;
        const stratTiming = $('#strategyTiming')?.value || 'early';
        if (stratTiming !== 'early') subParts[0] += ` · ${stratTiming} entry`;
      }
      ...
      let atSub = `<span style="color:${pnlColor}">${fmtPnl(tPnlEst)}</span> est P&L · Sell target: ${at0.sell_price_c||0}¢`;
      subParts.push(atSub);

    } else {
      // ── RUNNING but no trade ──
      dotClass = 'dot-yellow';
      const _hasShadow = !!(s.active_shadow && s.active_shadow.trade_id);
      ...
      if (detail.includes('Observing')) {
        if (_hasShadow) {
          statusMain = tradingMode === 'hybrid' ? 'Hybrid · Shadow Fallback' : 'Shadow Trading';
          statusColor = tradingMode === 'hybrid' ? 'var(--blue)' : '#a371f7';
          dotClass = 'dot-purple';
        } else {
          statusMain = _modeStatusLabel;
          statusColor = tradingMode === 'hybrid' ? 'var(--blue)' : tradingMode === 'shadow' ? '#a371f7' : 'var(--yellow)';
          if (tradingMode === 'shadow') dotClass = 'dot-purple';
        }
        const obsMatch = detail.match(/Observing (.+?) — next in ~(.+)/);
        if (obsMatch) {
          const _skipAS = s.active_skip || {};
          const _lmAS = s.live_market || {};
          const _obsN = _skipAS.regime_obs_n || _lmAS.regime_obs_n || 0;
          const _skipWhy = _skipAS.skip_short || '';
          const _parts = [];
          if (_hasShadow) {
            const _shSide = (s.active_shadow.side || '').toUpperCase();
            const _shPrice = s.active_shadow.price_c || 0;
            _parts.push(`${_shSide}@${_shPrice}¢ · 1 contract`);
            if (_obsN > 0 && !(_skipWhy && _skipWhy.includes('n='))) _parts.push(`n=${_obsN}`);
            if (tradingMode === 'hybrid' && _skipWhy) _parts.push(_skipWhy);
          } else {
            if (_obsN > 0 && !_skipWhy.includes('n=')) _parts.push(`n=${_obsN}`);
            if (_skipWhy) _parts.push(_skipWhy);
          }
          const _infoLine = _parts.length ? _parts.join(' · ') : '';
          const _countdown = `next in ~${obsMatch[2]}`;
          subParts.push(_infoLine ? `${_infoLine}<br>${_countdown}` : _countdown);
        } else {
          subParts.push(detail.replace('Observing ', ''));
        }
      } else if (detail.includes('Observed ')) {
        statusMain = _isDataMode ? _modeStatusLabel : 'Observed';
        statusColor = _isDataMode ? (tradingMode === 'shadow' ? '#a371f7' : 'var(--yellow)') : 'var(--yellow)';
        const obsMatch = detail.match(/Observed (.+?) — next in ~(.+)/);
        if (obsMatch) {
          subParts.push(`${obsMatch[1]} · next in ~${obsMatch[2]}`);
        } else {
          subParts.push(detail.replace('Observed ', '').replace('Observed: ', ''));
        }
      } else if (detail.includes('Observed:')) {
        statusMain = _isDataMode ? _modeStatusLabel : 'Observed';
        statusColor = _isDataMode ? (tradingMode === 'shadow' ? '#a371f7' : 'var(--yellow)') : 'var(--yellow)';
        const reason = detail.replace('Observed: ', '').replace(/ — next in ~.*/, '');
        const countdown = detail.match(/next in ~(.+?)(?:\s*·|$)/);
        subParts.push(reason);
        if (countdown) subParts.push(`next in ~${countdown[1]}`);
      } else if (detail.includes('Skipped')) {
        statusMain = _isDataMode ? _modeStatusLabel : 'Observed';
        statusColor = _isDataMode ? (tradingMode === 'shadow' ? '#a371f7' : 'var(--yellow)') : 'var(--yellow)';
        const reason = detail.replace(/Skipped /, '').replace(/ — next in ~.*/, '');
        const countdown = detail.match(/next in ~(.+?)(?:\s*·|$)/);
        subParts.push(reason);
        if (countdown) subParts.push(`next in ~${countdown[1]}`);
        ...
        statusColor = 'var(--yellow)';
      } else if (detail.includes('Resolving') || (detail.includes('Market closed') && !detail.includes('waiting for next'))) {
        ...
        statusMain = 'Starting...';
        statusColor = 'var(--blue)';
      } else if (detail.includes('Between') || detail.includes('waiting for next') || detail.includes('Next market') || detail.includes('Awaiting') || detail.includes('Idle') || detail.includes('recording market data') || !detail || detail === 'Idle — press play to start' || detail === 'Bot ready') {
        const nmNext = lastStateData._nextMarketOpen;
        if (nmNext) {
          const diff = Math.max(0, (new Date(nmNext) - new Date()) / 1000);
          if (diff > 0 && diff < 1200) {
            statusMain = _isDataMode ? `${_modeStatusLabel} · next in ${fmtMmSs(diff)}` : `Next market in ${fmtMmSs(diff)}`;
          } else if (diff <= 0) {
            statusMain = _isDataMode ? `${_modeStatusLabel} · market starting` : 'Market starting...';
          } else {
    ...
    // Show last trade result when not in a trade (skip if statusMain already shows result)
    const _statusShowsResult = statusMain.includes('WIN') || statusMain.includes('LOSS') || statusMain.includes('MAX LOSS');
    if (!hasActiveTrade && !hasPendingTrade && !s.cashing_out) {
      // Cache last completed trade so it persists after dismiss
      const lt = s.last_completed_trade;
      if (lt && lt.outcome) lastStateData._lastTrade = lt;
      const cached = lastStateData._lastTrade;
      if (cached && cached.outcome && !_statusShowsResult) {
        const ltPnl = cached.pnl || 0;
        const ltOut = cached.outcome === 'win' ? 'WIN' : cached.outcome === 'loss' ? 'LOSS' :
                      cached.outcome === 'cashed_out' ? 'CASHED OUT' : cached.outcome.toUpperCase();
        const ltColor = ltPnl >= 0 ? 'var(--green)' : 'var(--red)';
        subParts.unshift(`<span style="color:${ltColor}">Last: ${ltOut} ${fmtPnl(ltPnl)}</span>`);
      }
    } else {
      // Clear cached last trade when entering a new trade
      if (hasActiveTrade || hasPendingTrade) lastStateData._lastTrade = null;
    }

    // Append mode + bet info when running and sub is sparse (not in data-only modes)
    if (isRunning && tradingMode !== 'observe' && tradingMode !== 'shadow' && subParts.length < 2) {
    ...
    $('#hdrBal').textContent = '$' + effBal.toFixed(2);
    const hdrSpnl = s.session_pnl || 0;
    const hdrPnlEl = $('#hdrPnl');
    if (hdrSpnl !== 0) {
      hdrPnlEl.textContent = fmtPnl(hdrSpnl);
      hdrPnlEl.className = hdrSpnl > 0 ? 'pos' : 'neg';
      hdrPnlEl.style.fontWeight = '600';
    } else {
      hdrPnlEl.textContent = '';
      hdrPnlEl.className = 'dim';
    }

    // Running timer
    const timerEl = $('#hdrTimer');
    if (isRunning) {
      if (!_autoTradingSince) {
        _autoTradingSince = Date.now();
        try { sessionStorage.setItem('_autoSince', String(_autoTradingSince)); } catch(e) {}
      }
      const elapsed = Math.max(0, Math.floor((Date.now() - _autoTradingSince) / 1000));
      ...
      if (_autoTradingSince) {
        _autoTradingSince = null;
        try { sessionStorage.removeItem('_autoSince'); } catch(e) {}
      }
    ...
    // Detect new trade results for streak tracking
    // Also detect server session reset (counts dropped)
    if (_sessTrack.prevWins >= 0) {
      if (sw < _sessTrack.prevWins || sl < _sessTrack.prevLosses) {
        // Server reset session — reset client tracking too
        _sessTrack.curStreakType = null;
        _sessTrack.curStreakLen = 0;
        _sessTrack.maxWinStreak = 0;
        _sessTrack.maxLossStreak = 0;
        _sessTrack.peakPnl = 0;
      } else {
        if (sw > _sessTrack.prevWins) {
          if (_sessTrack.curStreakType === 'win') {
            _sessTrack.curStreakLen++;
          } else {
            _sessTrack.curStreakType = 'win';
            _sessTrack.curStreakLen = 1;
          }
          _sessTrack.maxWinStreak = Math.max(_sessTrack.maxWinStreak, _sessTrack.curStreakLen);
        }
        if (sl > _sessTrack.prevLosses) {
          if (_sessTrack.curStreakType === 'loss') {
            _sessTrack.curStreakLen++;
          } else {
            _sessTrack.curStreakType = 'loss';
            _sessTrack.curStreakLen = 1;
          }
          _sessTrack.maxLossStreak = Math.max(_sessTrack.maxLossStreak, _sessTrack.curStreakLen);
        }
      }
    }
    _sessTrack.prevWins = sw;
    _sessTrack.prevLosses = sl;

    // Peak P&L / drawdown
    if (spnl2 > _sessTrack.peakPnl) _sessTrack.peakPnl = spnl2;
    const drawdown = _sessTrack.peakPnl - spnl2;

    // Row 1: Wins, Losses, Win Rate, P&L
    $('#statWins').textContent = sw;
    $('#statLosses').textContent = sl;
    const sWR = sTotal > 0 ? (sw / sTotal * 100).toFixed(0) + '%' : '—';
    const wrEl = $('#statWinRate');
    wrEl.textContent = sWR;
    wrEl.className = 'val' + (sTotal > 0 ? (sw/sTotal >= 0.6 ? ' pos' : sw/sTotal < 0.45 ? ' neg' : '') : '');
    const spEl = $('#statSessionPnl');
    spEl.textContent = fmtPnl(spnl2);
    spEl.className = 'val ' + (spnl2 > 0 ? 'pos' : spnl2 < 0 ? 'neg' : '');

    // Row 2: Win Streak, Loss Streak, Peak P&L, Drawdown
    $('#statWinStreak').textContent = _sessTrack.maxWinStreak;
    $('#statLossStreak').textContent = _sessTrack.maxLossStreak;
    const peakEl = $('#statPeakPnl');
    peakEl.textContent = _sessTrack.peakPnl > 0 ? fmtPnl(_sessTrack.peakPnl) : '—';
    peakEl.className = 'val' + (_sessTrack.peakPnl > 0 ? ' pos' : '');
    const ddEl = $('#statDrawdown');
    ddEl.textContent = drawdown > 0 ? '-$' + drawdown.toFixed(2) : '—';
    ddEl.className = 'val' + (drawdown > 0 ? ' neg' : '');

    // Row 3: Trades, Skips, Avg P&L
    $('#statTrades').textContent = sTotal;
    const avgPnl = sTotal > 0 ? spnl2 / sTotal : 0;
    const avgEl = $('#statAvgPnl');
    if (sTotal > 0) {
      avgEl.textContent = fmtPnl(avgPnl);
      avgEl.className = 'val ' + (avgPnl > 0 ? 'pos' : avgPnl < 0 ? 'neg' : '');
    } else { avgEl.textContent = '—'; avgEl.className = 'val'; }
    $('#statSkips').textContent = sSkips;

    // Auto-fit stat values that overflow
    _autoFitStatVals();

    // Show recover button if previous session exists
    const recBtn = document.getElementById('recoverSessionBtn');
    if (recBtn) {
      const hasPrev = s._prev_session && s._prev_session !== '';
      recBtn.style.display = hasPrev ? '' : 'none';
    }
    ...
    if (at) {
      ...
      if (!isCashingOut) {
        ...
        const coSold = at.sell_progress || 0;
        const coRemaining = coShares - coSold;
        // Estimate: sell remaining at current bid (fees ~7%)
        const coGross = coRemaining * coBid / 100;
        const coAlreadySold = coSold * (at.sell_price_c || 0) / 100;
        const coTotal = coGross + coAlreadySold;
        const coPnl = coTotal - coCost;
        const coBtn = $('#cashOutBtn');
        const coEst = $('#cashOutEstimate');
        if (coBid > 0 && coRemaining > 0) {
          const pnlColor = coPnl >= 0 ? 'var(--green)' : 'var(--red)';
          coEst.innerHTML = `~$${coTotal.toFixed(2)} <span style="color:${pnlColor}">(${fmtPnl(coPnl)})</span>`;
          coBtn.className = coPnl >= 0 ? 'act-btn act-btn-green' : 'act-btn act-btn-red';
        } else {
          coEst.textContent = '';
          coBtn.className = 'act-btn act-btn-red';
        }
      }
...
async function loadConfig() {
  ...
  if (cfg.session_loss_limit) $('#sessionLossLimit').value = cfg.session_loss_limit;
  if (cfg.rolling_wr_window) $('#rollingWrWindow').value = cfg.rolling_wr_window;
  if (cfg.rolling_wr_floor) $('#rollingWrFloor').value = cfg.rolling_wr_floor;
  ...
  if (cfg.auto_lock_random !== undefined) $('#autoLockRandom').checked = cfg.auto_lock_random;
  if (cfg.auto_lock_amount) $('#autoLockAmount').value = cfg.auto_lock_amount;
  if (cfg.profit_goal) $('#profitGoal').value = cfg.profit_goal;
  ...
  if (cfg.push_notify_auto_lock !== undefined) $('#notifyAutoLock').checked = cfg.push_notify_auto_lock;
...
async function _loadApiKeyStatus() {
  try {
    ...
    if (r.set) {
      const src = r.source === 'dashboard' ? 'saved in settings' : 'from .env';
      el.innerHTML = `<span style="color:var(--green)">✓ Active</span> <span class="dim">(${r.preview} · ${src})</span>`;
    } else {
      el.innerHTML = '<span style="color:var(--red)">✗ Not set</span> — AI chat requires an Anthropic API key';
    }
  } catch(e) { console.error('API key status error:', e); }
}

async function _saveApiKey() {
  const input = $('#apiKeyInput');
  const key = (input.value || '').trim();
  if (!key) { showToast('Enter an API key', 'red'); return; }
  if (!key.startsWith('sk-')) { showToast('Key should start with sk-', 'orange'); return; }
  try {
    const r = await api('/api/apikey', {
      method: 'POST', headers:{'Content-Type':'application/json','X-CSRF-Token':_getCsrfToken()},
...
function _getTradeFilterParams() {
  const filters = _tradeFilters.has('all') ? 'all' : [..._tradeFilters].join(',');
...
function _renderFilterStats(stats) {
  ...
  const key = stats.total + '|' + stats.wins + '|' + stats.losses + '|' + stats.pnl + '|' + (stats.errors||0);
...
async function loadTrades() {
  ...
  const {filters, regime} = _getTradeFilterParams();
  try {
    const d = await api(`/api/trades_v2?filters=${encodeURIComponent(filters)}&regime=${encodeURIComponent(regime)}&offset=0&limit=${_TRADE_PAGE_SIZE}`);
    hideSkel('skelTrades');
...
async function loadMoreTrades() {
  ...
  const {filters, regime} = _getTradeFilterParams();
  try {
    const d = await api(`/api/trades_v2?filters=${encodeURIComponent(filters)}&regime=${encodeURIComponent(regime)}&offset=${_tradeOffset}&limit=${_TRADE_PAGE_SIZE}`);
    if (d.trades.length) {
      const el = $('#tradeList');
...
if (['win', 'loss', 'cashed_out', 'open'].includes(o) || isShadow) {
...
const isReal = ['win', 'loss', 'cashed_out', 'open'].includes(o) || isShadow;
...
async function showTradeDetail(tradeId) {
  try {
    ...
    const _isRealTrade = ['win', 'loss', 'cashed_out', 'open'].includes(o);
...
const _pushTagColors = {
  ...
  'cash-out': 'var(--orange)', 'error': 'var(--red)', 'bankroll-limit': 'var(--red)',
  'session-target': 'var(--green)', 'deploy': 'var(--dim)',
  'auto-scale': 'var(--blue)',
};
...
async function showPushLog(filterTag) {
  try {
    ...
    const tags = ['trade-result', 'buy', 'skip', 'max-loss', 'loss-stop', 'auto-lock',
                  'early-exit', 'cash-out', 'withdrawal', 'health-check', 'auto-scale', 'error', 'deploy'];
...
async function loadRegimes(force) {
  // Skip periodic background refresh if not on Bitcoin tab or regime detail modal is open
  if (!force && _currentTab !== 'Bitcoin') return;
  if (!force) {
...
function _applyQuickTradeToConfig() {
  ...
  _updateRiskActionLock();
  // Refresh regime list on Bitcoin tab to show updated overrides
  loadRegimes(true);
}
...
async function _loadLifetimeStatsInner() {
  try {
    ...
    // Observatory preview (separate call)
    api('/api/observatory').then(obs => {
      const oEl = document.getElementById('hubObsPreview');
      if (oEl && obs && obs.observations) oEl.innerHTML = `${obs.observations.total||0} markets recorded`;
    }).catch(() => {});

    // Model preview
    api('/api/fv_model_status').then(fv => {
      const mEl = document.getElementById('hubModelPreview');
      if (mEl && fv) mEl.innerHTML = fv.ready ? `<span class="pos">● Ready</span> · ${fv.cells_loaded||0} cells` : `<span class="dim">○ Building (${fv.cells_loaded||0} cells)</span>`;
    }).catch(() => {});

    // Validation preview
    api('/api/realized_edge').then(re => {
      const vEl = document.getElementById('hubValPreview');
      if (vEl && re) {
        const biggest = re.windows ? (re.windows[200] || re.windows[100] || re.windows[50]) : null;
        if (biggest) vEl.innerHTML = `Edge: <span class="${biggest.avg_pnl_per_trade_c>=0?'pos':'neg'}">${biggest.avg_pnl_per_trade_c>=0?'+':''}${biggest.avg_pnl_per_trade_c}¢/trade</span> (n=${biggest.n_trades})`;
      }
    }).catch(() => {});

    // Convergence preview
    api('/api/data_convergence?hours=24').then(cv => {
      const cEl = document.getElementById('hubConvPreview');
      if (cEl && cv && cv.has_data) {
        const s = cv.overall_pct || 0;
        const clr = s >= 85 ? 'pos' : s >= 60 ? '' : 'dim';
        cEl.innerHTML = `Stability: <span class="${clr}" style="font-weight:700">${s}%</span> <span class="dim">(24h)</span>`;
      }
    }).catch(() => {});
...
async function _statsLoadPage(page) {
  ...
  try {
    switch(page) {
      ...
      case 'convergence': await _statsRenderConvergence(el); break;
...
async function _statsRenderPerformance(el) {
  ...
  core += _sRow('Record', `${w}W – ${l}L` + (s.cashouts ? ` – ${s.cashouts} CO` : ''));
...
async function _statsRenderConditions(el) {
  ...
  // BTC Movement
  const bmb = s.btc_move_breakdown || [];
  let btcBody = '';
  if (bmb.length > 0 && bmb.some(r => r.btc_move_bucket !== 'N/A')) {
    btcBody = _sTable(_sStdHeaders.map((h,i) => i===0?{label:'BTC Move',left:true}:h),
      bmb.filter(r => r.btc_move_bucket !== 'N/A').map(r => {
        const rt=(r.wins||0)+(r.losses||0); const rwr=rt>0?((r.wins||0)/rt*100).toFixed(0):'—'; const rpnl=r.net_pnl||0;
        return `<tr><td>${r.btc_move_bucket}</td><td class="pos">${r.wins||0}</td><td class="neg">${r.losses||0}</td><td>${rwr}%</td><td class="${rpnl>=0?'pos':'neg'}">${fmtPnl(rpnl)}</td></tr>`;
      }).join('')
    );
  } else { btcBody = '<div class="dim">No BTC movement data yet</div>'; }
  html += _sSection('secBtcMove', 'BTC Movement', btcBody, true);
  ...
  // Stability
  const stab = s.stability_breakdown || [];
  let stabBody = '';
  if (stab.length > 0) {
    stabBody = _sTable(_sStdHeaders.map((h,i) => i===0?{label:'Stability',left:true}:h),
      stab.filter(r => r.stability_bucket !== 'N/A').map(r => {
        const rt=(r.wins||0)+(r.losses||0); const rwr=rt>0?((r.wins||0)/rt*100).toFixed(0):'—'; const rpnl=r.net_pnl||0;
        return `<tr><td>${r.stability_bucket}</td><td class="pos">${r.wins||0}</td><td class="neg">${r.losses||0}</td><td>${rwr}%</td><td class="${rpnl>=0?'pos':'neg'}">${fmtPnl(rpnl)}</td></tr>`;
      }).join('')
    );
  } else { stabBody = '<div class="dim">No stability data yet</div>'; }
  html += _sSection('secStability', 'Price Stability', stabBody, true);
...
async function _statsRenderObservatory(el) {
  const [obsData, neData, skipData, stateData, captureData] = await Promise.all([
    api('/api/observatory').catch(() => null),
    api('/api/net_edge').catch(() => null),
    api('/api/skip_analysis').catch(() => null),
    api('/api/state').catch(() => null),
    api('/api/capture_rate').catch(() => null),
  ]);
  let html = '';
  ...
  if (obsData && obsData.observations) {
    ...
    const totalObs = obs.total||0;
    if (totalObs > 0) {
      obsHtml += `<div class="opp-stat">
        <div class="opp-pill">Markets: <strong>${totalObs}</strong></div>
        <div class="opp-pill">Resolved: <strong>${obs.resolved||0}</strong></div>
        <div class="opp-pill">Traded: <strong>${obs.traded||0}</strong></div>
        <div class="opp-pill">Observed: <strong>${obs.observed||0}</strong></div>
      </div>`;
      const qFull = obs.quality_full||0, qPartial = obs.quality_partial||0, qShort = obs.quality_short||0, qFew = obs.quality_few||0;
      if (qPartial + qShort + qFew > 0)
        obsHtml += `<div style="font-size:10px;color:var(--dim);margin:4px 0">Quality: ${qFull} full · ${qPartial} partial · ${qShort} short · ${qFew} few</div>`;

      // Session health from bot_state
      let healthJson = stateData ? stateData.observatory_health : null;
      if (typeof healthJson === 'string') try { healthJson = JSON.parse(healthJson); } catch(e) { healthJson = null; }
      if (healthJson) {
        const dr = healthJson.drop_rate_pct || 0;
        const drColor = dr <= 5 ? 'var(--green)' : dr <= 20 ? 'var(--yellow)' : 'var(--red)';
        obsHtml += `<div style="display:flex;gap:8px;margin:8px 0;font-size:11px">
          <span>Session: <strong>${healthJson.written||0}</strong> written</span>
          <span><strong>${healthJson.dropped_partial||0}</strong> partial</span>
          <span><strong>${healthJson.dropped_short||0}</strong> short</span>
          <span>Drop rate: <strong style="color:${drColor}">${dr}%</strong></span>
        </div>`;
      }

      // Capture rate
      if (captureData && captureData.capture_rate != null) {
        const cr = (captureData.capture_rate * 100).toFixed(1);
        const crColor = captureData.capture_rate >= 0.8 ? 'var(--green)' : captureData.capture_rate >= 0.5 ? 'var(--yellow)' : 'var(--red)';
        obsHtml += `<div style="font-size:11px;margin:4px 0">Capture rate (24h): <strong style="color:${crColor}">${cr}%</strong> (${captureData.captured||0}/${captureData.total_available||0} markets)</div>`;
      }

      // Top strategies
      const top = obsData.top_strategies || [];
      if (top.length > 0) {
        obsHtml += '<div class="stat-category">Best Strategies (by EV)</div>';
        obsHtml += _sTable(
          [{label:'Setup',left:true},{label:'Strategy'},{label:'n'},{label:'Win%'},{label:'EV/trade'}],
          top.slice(0, 8).map(r => {
            const setup = (r.setup_key||'').replace(/^[^:]+:/, '').replace(/_/g, ' ');
            const strat = (r.strategy_key||'').replace(/:/g, ' · ');
            const ev = r.ev_per_trade_c != null ? r.ev_per_trade_c.toFixed(1)+'¢' : '—';
            const evCls = (r.ev_per_trade_c||0) > 0 ? 'pos' : 'neg';
            const wr2 = r.win_rate != null ? (r.win_rate*100).toFixed(0)+'%' : '—';
            const fdrBadge = r.fdr_significant ? '<span style="color:var(--green);font-size:8px;margin-left:2px">✓</span>' : '';
            return `<tr><td style="text-align:left;max-width:90px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${setup}</td><td style="font-size:10px;max-width:100px;overflow:hidden;text-overflow:ellipsis">${strat}${fdrBadge}</td><td>${r.sample_size||0}</td><td>${wr2}</td><td class="${evCls}">${ev}</td></tr>`;
          }).join('')
        );
      }
      const avoid = obsData.avoid_setups || [];
      if (avoid.length > 0) {
        obsHtml += '<div class="stat-category">Setups to Avoid</div>';
        avoid.slice(0, 5).forEach(r => {
          const setup = (r.setup_key||'').replace(/^[^:]+:/, '').replace(/_/g, ' ');
          const ev = r.ev_per_trade_c != null ? r.ev_per_trade_c.toFixed(1) : '—';
          obsHtml += `<div style="font-size:11px;margin:2px 0"><span class="neg">${ev}¢</span> · ${setup} (n=${r.sample_size||0})</div>`;
        });
      }
    } else { obsHtml = '<div class="dim">Collecting market data...</div>'; }
  } else { obsHtml = '<div class="dim">Observatory starting...</div>'; }
  html += _sSection('secObservatory', 'Strategy Observatory', obsHtml, false);

  // Net Edge Summary
  let neHtml = '';
  if (neData && !neData.error) {
    const sufficient = neData.data_sufficient;
    const totalObs2 = neData.total_observations||0;
    const needed = neData.min_observations_needed||200;
    if (!sufficient) {
      const pct = Math.min(100, Math.round(totalObs2/needed*100));
      neHtml += `<div style="padding:12px;background:rgba(210,153,34,0.1);border:1px solid rgba(210,153,34,0.3);border-radius:6px;margin-bottom:10px">
        <div style="font-weight:600;color:var(--yellow);margin-bottom:4px">Collecting Data</div>
        <div style="font-size:12px;color:var(--dim)">${totalObs2}/${needed} observations (${pct}%)</div>
        <div style="height:4px;background:var(--border);border-radius:2px;margin-top:6px;overflow:hidden"><div style="height:100%;width:${pct}%;background:var(--yellow);border-radius:2px"></div></div>
      </div>`;
    }
    const fdrCount = neData.fdr_significant_strategies||0;
    neHtml += `<div class="opp-stat">
      <div class="opp-pill">Observations: <strong>${totalObs2}</strong></div>
      <div class="opp-pill">Sim-eligible: <strong>${neData.sim_eligible_observations||0}</strong></div>
      <div class="opp-pill">Strategies: <strong>${neData.total_evaluated_strategies||0}</strong></div>
      <div class="opp-pill">FDR-sig: <strong style="color:${fdrCount>0?'var(--green)':'var(--dim)'}">${fdrCount}</strong></div>
    </div>`;
    const bestFdr = neData.best_fdr;
    if (bestFdr) {
      const ev = bestFdr.ev_per_trade_c;
      const evCls = ev > 0 ? 'pos' : 'neg';
      const fdrKey = (bestFdr.strategy_key||'').replace(/'/g, "\\'");
      const fdrActive = _strategyAutoFill && _strategyAutoFill.stratKey === bestFdr.strategy_key && _strategyAutoFill.regime === 'global';
      neHtml += `<div class="best-strat-row${fdrActive?' af-active':''}" data-strat-key="${bestFdr.strategy_key||''}" data-strat-regime="global" onclick="_applyStrategyAutoFill('global',1,'${fdrKey}')" style="margin:10px 0;padding:10px;border:1px solid var(--green);border-radius:6px;background:rgba(63,185,80,0.06)">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div style="font-size:10px;color:var(--green);text-transform:uppercase">Best FDR-Significant Strategy</div>
          <span class="af-status-label" style="font-size:9px;font-weight:700;color:${fdrActive?'#a371f7':'var(--dim)'}">${fdrActive ? 'ACTIVE' : 'tap to apply'}</span>
        </div>
        <div style="font-size:22px;font-weight:700;font-family:monospace;margin-top:4px" class="${evCls}">${ev>0?'+':''}${ev.toFixed(1)}\u00a2/trade</div>
        <div style="font-size:11px;color:var(--dim);margin-top:4px">${(bestFdr.strategy_key||'').replace(/:/g,' \u00b7 ')} \u00b7 WR ${(bestFdr.win_rate*100).toFixed(1)}% \u00b7 n=${bestFdr.sample_size}${bestFdr.fdr_q_value!=null?' \u00b7 q='+bestFdr.fdr_q_value.toFixed(3):''}</div>
        ${bestFdr.oos_ev_c != null ? `<div style="font-size:11px;color:var(--dim)">Out-of-sample EV: ${bestFdr.oos_ev_c>0?'+':''}${bestFdr.oos_ev_c.toFixed(1)}\u00a2 (n=${bestFdr.oos_sample_size||0})</div>` : ''}
      </div>`;
    } else if (neData.best_overall) {
      const best = neData.best_overall; const ev = best.ev_per_trade_c;
      const bestKey = (best.strategy_key||'').replace(/'/g, "\\'");
      const bestActive = _strategyAutoFill && _strategyAutoFill.stratKey === best.strategy_key && _strategyAutoFill.regime === 'global';
      neHtml += `<div class="best-strat-row${bestActive?' af-active':''}" data-strat-key="${best.strategy_key||''}" data-strat-regime="global" onclick="_applyStrategyAutoFill('global',1,'${bestKey}')" style="margin:10px 0;padding:10px;border:1px solid var(--border);border-radius:6px">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div style="font-size:10px;color:var(--yellow);text-transform:uppercase">Best Strategy (not yet FDR-validated)</div>
          <span class="af-status-label" style="font-size:9px;font-weight:700;color:${bestActive?'#a371f7':'var(--dim)'}">${bestActive ? 'ACTIVE' : 'tap to apply'}</span>
        </div>
        <div style="font-size:22px;font-weight:700;font-family:monospace;margin-top:4px" class="${ev>0?'pos':'neg'}">${ev>0?'+':''}${ev.toFixed(1)}\u00a2/trade</div>
        <div style="font-size:11px;color:var(--dim);margin-top:4px">${(best.strategy_key||'').replace(/:/g,' \u00b7 ')} \u00b7 n=${best.sample_size}</div>
      </div>`;
    }
  } else { neHtml = '<div class="dim">No net edge data available yet</div>'; }
  html += _sSection('secNetEdge', 'Net Edge Summary', neHtml, false);

  // Strategy Lab
  let labHtml = `<div class="dim" style="font-size:10px;margin-bottom:8px">~1,881 strategies simulated against every recorded market.</div>
    <div class="filter-chips" id="stratLabMode" style="margin-bottom:8px">
      <button class="chip active" onclick="_setStratLabMode('best',this)">Best Per Regime</button>
      <button class="chip" onclick="_setStratLabMode('lookup',this)">Strategy Lookup</button>
    </div>
    <div id="stratLabFilters" style="display:none;margin-bottom:8px">
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:4px;font-size:11px">
        <select id="slSide" onchange="_loadStrategyLab()" style="font-size:11px;padding:3px;background:var(--bg);border:1px solid var(--border);color:var(--text);border-radius:4px;width:100%">
          <option value="">Any Side</option><option value="cheaper">Cheaper</option>
          <option value="yes">YES</option><option value="no">NO</option><option value="model">Model</option>
        </select>
        <select id="slTiming" onchange="_loadStrategyLab()" style="font-size:11px;padding:3px;background:var(--bg);border:1px solid var(--border);color:var(--text);border-radius:4px;width:100%">
          <option value="">Any Time</option><option value="early">Early</option><option value="mid">Mid</option><option value="late">Late</option>
        </select>
        <select id="slEntry" onchange="_loadStrategyLab()" style="font-size:11px;padding:3px;background:var(--bg);border:1px solid var(--border);color:var(--text);border-radius:4px;width:100%">
          <option value="">Any Buy</option><option value="5">≤5¢</option><option value="10">≤10¢</option><option value="15">≤15¢</option>
        ...
        <select id="slSell" onchange="_loadStrategyLab()" style="font-size:11px;padding:3px;background:var(--bg);border:1px solid var(--border);color:var(--text);border-radius:4px;width:100%">
          <option value="">Any Sell</option><option value="hold">Hold</option>
          <option value="99">99¢</option><option value="95">95¢</option><option value="90">90¢</option>
          <option value="85">85¢</option><option value="80">80¢</option><option value="75">75¢</option>
          <option value="70">70¢</option><option value="65">65¢</option><option value="60">60¢</option>
        </select>
      </div>
    </div>
    <div id="stratLabContent"><div class="dim">Loading...</div></div>
    <button onclick="_recomputeStrategies()" class="stats-action-btn" style="margin-top:8px">Recompute All Strategies</button>`;
  html += _sSection('secStrategyLab', 'Strategy Lab <span class="ssh-sub" id="stratLabSub"></span>', labHtml, false);

  // Data Collection (skips)
  let skipHtml = '';
  if (skipData && skipData.overview) {
    const sk = skipData.overview;
    const totalSkipped = sk.total_skipped||0;
    if (totalSkipped > 0) {
      skipHtml += `<div class="opp-stat">
        <div class="opp-pill">Observed: <strong>${totalSkipped}</strong></div>
        <div class="opp-pill">Results known: <strong>${sk.with_result||0}</strong></div>
        <div class="opp-pill">YES: <strong>${sk.result_yes||0}</strong></div>
        <div class="opp-pill">NO: <strong>${sk.result_no||0}</strong></div>
      </div>`;
      const skByRegime = skipData.by_regime || [];
      if (skByRegime.length > 0) {
        skipHtml += '<div class="dim" style="font-size:10px;margin:8px 0 4px">BY REGIME</div>';
        skipHtml += _sTable(
          [{label:'Regime',left:true},{label:'n'},{label:'YES'},{label:'NO'}],
          skByRegime.filter(r => (r.n||0) >= 1).map(r => {
            return `<tr><td style="text-align:left;max-width:100px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(r.regime_label||'—').replace(/_/g,' ')}</td><td>${r.n}</td><td>${r.result_yes||0}</td><td>${r.result_no||0}</td></tr>`;
          }).join('')
        );
      }
    } else { skipHtml = '<div class="dim">No observed markets yet</div>'; }
  } else { skipHtml = '<div class="dim">No observation data available</div>'; }
  html += _sSection('secSkips', 'Data Collection', skipHtml, true);

  // Hold vs Sell + Fee Sensitivity (buttons to load on demand)
  html += _sSection('secHoldVsSell', 'Hold vs Sell', '<div id="holdVsSellContent"><button class="stats-action-btn" onclick="_loadHoldVsSell()">Load Analysis</button><div class="dim" style="font-size:10px;margin-top:4px">Do sell targets actually beat hold-to-expiry after fees?</div></div>', true);
  html += _sSection('secFeeSensitivity', 'Fee Sensitivity', '<div id="feeSensContent"><button class="stats-action-btn" onclick="_loadFeeSensitivity()">Load Analysis</button><div class="dim" style="font-size:10px;margin-top:4px">Stress-test top strategies at higher fee rates</div></div>', true);
...
// Hold vs Sell loader
async function _loadHoldVsSell() {
  const el = document.getElementById('holdVsSellContent');
  el.innerHTML = '<div class="dim">Loading...</div>';
  try {
    const d = await api('/api/hold_vs_sell');
    if (d.error) { el.innerHTML = `<div class="dim">${d.error}</div>`; return; }
    let h = '';
    const hold = d.hold || {}; const sell = d.sell || {};
    h += `<div style="display:flex;gap:12px;margin-bottom:10px">
      <div class="stat-summary-card" style="flex:1"><div class="ssc-val">${hold.ev_c != null ? (hold.ev_c>0?'+':'')+hold.ev_c.toFixed(1)+'¢' : '—'}</div><div class="ssc-label">Hold EV</div></div>
      <div class="stat-summary-card" style="flex:1"><div class="ssc-val">${sell.ev_c != null ? (sell.ev_c>0?'+':'')+sell.ev_c.toFixed(1)+'¢' : '—'}</div><div class="ssc-label">Sell EV</div></div>
    </div>`;
    if (d.verdict) h += `<div style="font-size:11px;padding:6px 8px;background:rgba(48,54,61,0.3);border-radius:6px">${d.verdict}</div>`;
    if (d.by_target && d.by_target.length > 0) {
      h += '<div class="stat-category" style="margin-top:10px">By Sell Target</div>';
      h += _sTable([{label:'Target',left:true},{label:'n'},{label:'Win%'},{label:'EV/trade'}],
        d.by_target.map(t => {
          const ev = t.ev_c != null ? t.ev_c.toFixed(1) : '—';
          const wr3 = t.win_rate != null ? (t.win_rate*100).toFixed(0) : '—';
          return `<tr><td>${t.target}¢</td><td>${t.n||0}</td><td>${wr3}%</td><td class="${(t.ev_c||0)>0?'pos':'neg'}">${ev}¢</td></tr>`;
        }).join('')
      );
    }
    el.innerHTML = h;
  } catch(e) { el.innerHTML = `<div class="dim" style="color:var(--red)">${e.message}</div>`; }
}

// Fee Sensitivity loader
async function _loadFeeSensitivity() {
  const el = document.getElementById('feeSensContent');
  el.innerHTML = '<div class="dim">Loading...</div>';
  try {
    const d = await api('/api/fee-sensitivity');
    if (d.error) { el.innerHTML = `<div class="dim">${d.error}</div>`; return; }
    const strats = d.strategies || [];
    if (strats.length === 0) { el.innerHTML = '<div class="dim">No strategies with enough data yet</div>'; return; }
    let h = '<div style="font-size:11px;color:var(--dim);margin-bottom:8px">How top strategies perform at higher fee rates (7% → 8% → 9%)</div>';
    h += _sTable([{label:'Strategy',left:true},{label:'7%'},{label:'8%'},{label:'9%'},{label:'Fragile?'}],
      strats.map(s2 => {
        const ev7 = s2.ev_7 != null ? s2.ev_7.toFixed(1) : '—';
        const ev8 = s2.ev_8 != null ? s2.ev_8.toFixed(1) : '—';
        const ev9 = s2.ev_9 != null ? s2.ev_9.toFixed(1) : '—';
        const fragile = s2.fee_fragile;
        return `<tr><td style="text-align:left;font-size:10px;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(s2.strategy_key||'').replace(/:/g,' · ')}</td>
          <td class="${(s2.ev_7||0)>0?'pos':'neg'}">${ev7}¢</td>
          <td class="${(s2.ev_8||0)>0?'pos':'neg'}">${ev8}¢</td>
          <td class="${(s2.ev_9||0)>0?'pos':'neg'}">${ev9}¢</td>
          <td>${fragile ? '<span class="neg">Yes</span>' : '<span class="pos">No</span>'}</td></tr>`;
      }).join('')
    );
    el.innerHTML = h;
  } catch(e) { el.innerHTML = `<div class="dim" style="color:var(--red)">${e.message}</div>`; }
...
// ────────────────────────────────────────────────────
//  MODELS & CALIBRATION PAGE
// ────────────────────────────────────────────────────
async function _statsRenderModels(el) {
  const [fvm, sfData, fi] = await Promise.all([
    api('/api/fv_model_status').catch(() => null),
    api('/api/btc_surface').catch(() => null),
    api('/api/feature_importance').catch(() => null),
  ]);
  let html = '';
  ...
  let fvHtml = '';
  if (fvm && !fvm.error) {
    const ready = fvm.ready;
    const cells = fvm.cells_loaded||0;
    const needed = fvm.min_cells_needed||8;
    const readyColor = ready ? 'var(--green)' : 'var(--yellow)';
    fvHtml += `<div style="display:flex;gap:12px;margin-bottom:10px">
      <div class="stat-summary-card" style="flex:1"><div class="ssc-val" style="color:${readyColor}">${ready?'● Ready':'○ Building'}</div><div class="ssc-label">Model Status</div></div>
      <div class="stat-summary-card" style="flex:1"><div class="ssc-val">${cells}/${needed}+</div><div class="ssc-label">Surface Cells</div></div>
    </div>`;
    const le = fvm.live_edge;
    if (le) {
      fvHtml += `<div style="padding:10px;border:1px solid var(--border);border-radius:6px;margin-bottom:8px">
        <div class="dim" style="font-size:10px;margin-bottom:4px">LIVE MARKET EDGE</div>
        <div style="font-size:11px">FV YES: <strong>${le.fair_value_yes}¢</strong> · Mkt: <strong>${le.market_yes_ask}¢</strong> · Edge: <span class="${(le.yes_edge_c||0)>0?'pos':'neg'}"><strong>${(le.yes_edge_c||0)>0?'+':''}${(le.yes_edge_c||0).toFixed(1)}¢</strong></span></div>
        ${le.recommended_side ? `<div style="font-size:11px;margin-top:2px">Rec: <strong class="${le.recommended_side==='yes'?'side-yes':'side-no'}">${le.recommended_side.toUpperCase()}</strong> · conf: ${((le.confidence||0)*100).toFixed(0)}%</div>` : ''}
      </div>`;
    }
  } else { fvHtml = '<div class="dim">Fair value model loading...</div>'; }
  html += _sSection('secFVModel', 'Fair Value Model', fvHtml, false);

  // BTC Probability Surface
  let sfHtml = '';
  if (sfData && !sfData.error) {
    const cells2 = sfData.cells || [];
    if (cells2.length > 0) {
      const timeBuckets = [...new Set(cells2.map(c => c.time_bucket))];
      const distBuckets = [...new Set(cells2.map(c => c.distance_bucket))];
      const lookup = {};
      cells2.forEach(c => { lookup[c.distance_bucket + '|' + c.time_bucket] = c; });
      sfHtml = '<div style="font-size:11px;color:var(--dim);margin-bottom:8px">YES win probability by BTC distance × time into market. Green=YES favored, Red=NO favored.</div>';
      sfHtml += '<div style="overflow-x:auto"><table class="proj-table" style="font-size:10px"><thead><tr><th style="text-align:left">Distance</th>';
      timeBuckets.forEach(tb => { sfHtml += `<th>${tb}</th>`; });
      sfHtml += '<th style="border-left:1px solid var(--border)">Edge</th></tr></thead><tbody>';
      distBuckets.forEach(db => {
        sfHtml += `<tr><td style="text-align:left;white-space:nowrap">${db}</td>`;
        let rowEdgeSum = 0, rowEdgeN = 0;
        timeBuckets.forEach(tb => {
          const cell = lookup[db + '|' + tb];
          if (cell && cell.total >= 5) {
            const wrN = cell.yes_win_rate;
            const deviation = Math.abs(wrN - 0.5);
            const nConf = Math.min(1.0, cell.total / 100);
            const alpha = (0.08 + deviation * 0.6) * (0.3 + nConf * 0.7);
            const bg2 = wrN > 0.5 ? `rgba(63,185,80,${alpha.toFixed(2)})` : `rgba(248,81,73,${alpha.toFixed(2)})`;
            if (cell.avg_yes_price != null && wrN > 0) { rowEdgeSum += (wrN*100 - cell.avg_yes_price); rowEdgeN++; }
            sfHtml += `<td style="background:${bg2};text-align:center;font-weight:${deviation>0.15?'600':'400'}" title="n=${cell.total}">${(wrN*100).toFixed(0)}%</td>`;
          } else { sfHtml += '<td style="color:var(--border);text-align:center">—</td>'; }
        });
        if (rowEdgeN > 0) {
          const avgEdge = rowEdgeSum / rowEdgeN;
          sfHtml += `<td style="border-left:1px solid var(--border);text-align:center;color:${avgEdge>2?'var(--green)':avgEdge<-2?'var(--red)':'var(--dim)'};font-weight:600">${avgEdge>=0?'+':''}${avgEdge.toFixed(1)}¢</td>`;
        } else { sfHtml += '<td style="border-left:1px solid var(--border);text-align:center;color:var(--border)">—</td>'; }
        sfHtml += '</tr>';
      });
      sfHtml += '</tbody></table></div>';
    } else { sfHtml = '<div class="dim">Need more observations to build probability surface.</div>'; }
  } else { sfHtml = '<div class="dim">Surface data not available yet.</div>'; }
  html += _sSection('secBtcSurface', 'BTC Probability Surface', sfHtml, false);

  // Feature Importance
  let fiHtml = '';
  if (fi && !fi.error) {
    const feats = fi.features || [];
    if (feats.length > 0) {
      fiHtml = '<div style="font-size:11px;color:var(--dim);margin-bottom:8px">Which features predict YES/NO outcomes. Higher = more predictive.</div>';
      fiHtml += _sTable(
        [{label:'Feature',left:true},{label:'Importance'},{label:'Corr'},{label:'n'}],
        feats.slice(0, 15).map(f => {
          const imp = (f.importance * 100).toFixed(1);
          const corr = f.correlation > 0 ? '+'+f.correlation.toFixed(3) : f.correlation.toFixed(3);
          const corrCls = Math.abs(f.correlation) > 0.05 ? (f.correlation > 0 ? 'pos' : 'neg') : '';
          const impBar = `<div style="display:inline-block;width:${Math.min(100, f.importance*500)}px;height:8px;background:var(--accent);border-radius:2px;vertical-align:middle;margin-left:4px"></div>`;
          return `<tr><td style="text-align:left">${f.feature_name.replace(/_/g,' ')}</td><td>${imp}% ${impBar}</td><td class="${corrCls}">${corr}</td><td>${f.sample_size}</td></tr>`;
        }).join('')
      );
    } else { fiHtml = '<div class="dim">Feature analysis runs hourly once you have 50+ observations.</div>'; }
  } else { fiHtml = '<div class="dim">No feature data available yet.</div>'; }
  html += _sSection('secFeatures', 'Feature Importance', fiHtml, true);
...
//  VALIDATION & EXECUTION PAGE
// ────────────────────────────────────────────────────
var _valSummary = null;  // cached summary data
var _valTestResults = {};  // cached test results: {persistence: {...}, permutation: {...}, ...}

async function _statsRenderValidation(el) {
  // Load the lightweight summary for the readiness overview
  _valSummary = await api('/api/validation_summary').catch(() => null);
  const vs = _valSummary;
  let html = '';

  // ═══ READINESS SCORE ═══
  html += _valBuildReadiness(vs);

  // ═══ DATA COLLECTION PROGRESS ═══
  html += _valBuildDataProgress(vs);

  // ═══ STRATEGY GATES ═══
  html += _valBuildGates(vs);

  // ═══ VALIDATION TESTS ═══
  html += '<div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;color:var(--blue);margin:16px 0 8px;padding-bottom:4px;border-bottom:1px solid rgba(88,166,255,0.2)">Validation Tests</div>';

  // Persistence Test
  const persThresh = vs?.thresholds?.persistence || {need:100, have:0};
  html += _valTestCard('persistence', 'Strategy Persistence', 
    'Splits your observation history in half chronologically. If the top strategies from the first half remain profitable in the second half, your edge is probably real — not just overfitting to past data.',
    persThresh, 'Run Test', '~15 sec');

  // Permutation Test
  const permThresh = vs?.thresholds?.permutation || {need:50, have:0};
  html += _valTestCard('permutation', 'Permutation Test',
    'The gold standard for edge validation. Randomly shuffles win/loss outcomes 500 times, then checks if your best strategy\'s EV beats 95% of the shuffled results. If yes, your edge is statistically significant and not just noise from testing thousands of variants.',
    permThresh, 'Run Test', '~30-60 sec');

  // Walk-Forward Selection
  const wfThresh = vs?.thresholds?.walkforward || {need:100, have:0};
  html += _valTestCard('walkforward', 'Walk-Forward Selection',
    'The ultimate pre-live test. Simulates what happens if you let the auto-strategy advisor pick strategies each period and trade them the next. If the advisor\'s picks are profitable out-of-sample, auto-strategy can be trusted.',
    wfThresh, 'Run Test', '~60-120 sec');

  // Add reset button if selection test is currently failed/set
  const _selResult = vs?.gate_status?.selection_test;
  if (_selResult === 'failed' || _selResult === 'passed') {
    html += `<div style="margin:-8px 0 12px;padding:8px 10px;background:${_selResult==='failed'?'rgba(248,81,73,0.06)':'rgba(63,185,80,0.06)'};border:1px solid ${_selResult==='failed'?'rgba(248,81,73,0.15)':'rgba(63,185,80,0.15)'};border-radius:6px;font-size:10px;display:flex;align-items:center;justify-content:space-between">
      <span style="color:${_selResult==='failed'?'var(--red)':'var(--green)'}">Selection test: <strong>${_selResult.toUpperCase()}</strong>${_selResult==='failed'?' — blocking all auto-strategy trades':''}</span>
      <button class="val-run-btn" style="font-size:9px;padding:3px 8px" onclick="_valResetSelection()">Clear</button>
    </div>`;
  }

  // ═══ EXECUTION ANALYSIS ═══
  html += '<div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;color:var(--blue);margin:16px 0 8px;padding-bottom:4px;border-bottom:1px solid rgba(88,166,255,0.2)">Execution Analysis</div>';

  // Realized Edge
  const reThresh = vs?.thresholds?.realized_edge || {need:1, have:0};
  html += _valTestCard('realized_edge', 'Realized Edge',
    'Compares your actual trade P&L to what the simulation predicted. If simulations say +3¢/trade but reality is -1¢/trade, execution is eating your edge. The gap between these numbers is the "execution tax."',
    reThresh, 'Load', 'instant');

  // P&L Attribution
  const pnlThresh = vs?.thresholds?.pnl_attribution || {need:5, have:0};
  html += _valTestCard('pnl_attribution', 'P&L Attribution',
    'Breaks down where your profit/loss comes from: model prediction quality, execution costs (slippage/fees), exit timing decisions, and side selection. Shows which component is helping or hurting the most.',
    pnlThresh, 'Load', 'instant');

  // Shadow Trade Analysis
  const shadThresh = vs?.thresholds?.shadow || {need:1, have:0};
  html += _valTestCard('shadow', 'Shadow Trade Analysis',
    'Compares simulated fills against real 1-contract shadow trades. Measures actual fill slippage, order latency, and whether simulation outcomes match reality. This directly calibrates how much the sim-to-live gap costs you.',
    shadThresh, 'Load', 'instant');

  // Capture Rate
  html += _valTestCard('capture_rate', 'Observation Capture Rate',
    'Checks what percentage of available 15-minute markets the bot actually recorded. Below 90% means gaps from restarts, deploys, or outages that can skew your data. 96 markets are expected per day.',
    {need:0, have:1}, 'Load', 'instant');

  el.innerHTML = html;
}

function _valBuildReadiness(vs) {
  const score = vs?.readiness_score ?? 0;
  const color = score >= 70 ? 'var(--green)' : score >= 40 ? 'var(--yellow)' : 'var(--red)';
  const verdict = score >= 70 ? 'Getting Close' : score >= 40 ? 'Building Data' : 'Early Stage';
  const sub = score >= 70 ? 'Strategy validation gates nearing completion'
    : score >= 40 ? 'Collecting observations — keep the bot running'
    : 'Need more observations before strategies can be validated';

  // SVG ring
  const r = 42, circ = 2 * Math.PI * r;
  const offset = circ - (score / 100) * circ;

  return `<div class="val-readiness">
    <div class="val-ring">
      ...
      <div class="val-ring-label" style="color:${color}">${score}</div>
    </div>
    <div class="val-verdict" style="color:${color}">${verdict}</div>
    <div class="val-sub">${sub}</div>
  </div>`;
}

function _valBuildDataProgress(vs) {
  if (!vs) return '';
  const obs = vs.total_observations || 0;
  const rate = vs.daily_rate || 0;
  const trades = vs.trade_count || 0;
  const cap = vs.capture_24h || 0;
  const exp = vs.expected_24h || 96;
  const capPct = exp > 0 ? Math.round(cap / exp * 100) : 0;
  const capColor = capPct >= 90 ? 'var(--green)' : capPct >= 70 ? 'var(--yellow)' : 'var(--red)';

  // Progress toward 200 obs (full validation threshold)
  const obsPct = Math.min(100, Math.round(obs / 200 * 100));
  const obsColor = obs >= 200 ? 'var(--green)' : obs >= 100 ? 'var(--yellow)' : 'var(--blue)';

  return `<div class="val-test-card" style="margin-bottom:12px">
    <div style="padding:10px 12px">
      <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:8px">Data Collection</div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:10px">
        <div style="text-align:center">
          <div style="font-size:18px;font-weight:700;font-family:monospace;color:${obsColor}">${obs}</div>
          <div style="font-size:9px;color:var(--dim);text-transform:uppercase">Observations</div>
        </div>
        <div style="text-align:center">
          <div style="font-size:18px;font-weight:700;font-family:monospace">${rate}</div>
          <div style="font-size:9px;color:var(--dim);text-transform:uppercase">Per Day</div>
        </div>
        <div style="text-align:center">
          <div style="font-size:18px;font-weight:700;font-family:monospace;color:${capColor}">${capPct}%</div>
          <div style="font-size:9px;color:var(--dim);text-transform:uppercase">Capture (24h)</div>
        </div>
      </div>
      <div class="val-req-bar">
        <div class="val-req-label"><span>Progress toward full validation (200 obs)</span><span>${obs}/200</span></div>
        <div class="val-req-track"><div class="val-req-fill" style="width:${obsPct}%;background:${obsColor}"></div></div>
      </div>
      ${trades > 0 ? `<div style="font-size:10px;color:var(--dim);margin-top:4px">${trades} real trade${trades!==1?'s':''} completed</div>` : ''}
    </div>
  </div>`;
}

function _valBuildGates(vs) {
  if (!vs || !vs.gates || Object.keys(vs.gates).length === 0) {
    return `<div class="val-test-card" style="margin-bottom:12px"><div style="padding:12px">
      <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:6px">Strategy Gates</div>
      <div class="val-desc">No strategies with positive EV yet. Keep collecting data — the Observatory tests strategy variants automatically every 30 minutes.</div>
    </div></div>`;
  }
  ...
  const strat = (vs.best_strategy || '').replace(/:/g, ' · ');

  const gateLabels = {
    positive_ev: 'Positive EV',
    oos_positive: 'OOS Positive',
    oos_sufficient: 'OOS Samples',
    fee_resilient: 'Fee Resilient',
    fdr_significant: 'FDR Significant',
  };
  const gateDescs = {
    positive_ev: 'Time-weighted expected value is above zero',
    oos_positive: 'Out-of-sample EV is positive (not just in-sample)',
    oos_sufficient: 'Enough out-of-sample data points to be confident',
    fee_resilient: 'Strategy survives even if fees increase slightly',
    fdr_significant: 'Passed false discovery rate correction across all variants',
  };

  let gateHtml = '<div class="val-checklist">';
  for (const [key, g] of Object.entries(gates)) {
    const icon = g.passed ? '✓' : '✗';
    const cls = g.passed ? 'pass' : 'fail';
    gateHtml += `<div class="val-check-item">
      <div class="val-check-icon ${cls}">${icon}</div>
      <div>
        <div style="font-size:11px;font-weight:500">${gateLabels[key] || key}</div>
        <div style="font-size:9px;color:var(--dim)">${g.detail || gateDescs[key] || ''}</div>
      </div>
    </div>`;
  }
  gateHtml += '</div>';

  return `<div class="val-test-card" style="margin-bottom:12px"><div style="padding:10px 12px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
      <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px">Strategy Gates</div>
      <div style="font-size:11px;font-weight:600;color:${passed===total?'var(--green)':'var(--yellow)'}">${passed}/${total} Passed</div>
    </div>
    <div style="font-size:10px;color:var(--dim);margin-bottom:8px">Best strategy: <span style="color:var(--text);font-family:monospace;font-size:10px">${strat || 'none'}</span></div>
    ${gateHtml}
    ${passed < total ? `<div style="font-size:10px;color:var(--dim);margin-top:8px">A strategy must pass all ${total} gates before auto-strategy will trade it.</div>` : '<div style="font-size:11px;color:var(--green);margin-top:8px;font-weight:600;text-align:center">All gates passed — run the tests below to confirm the edge is real.</div>'}
  </div></div>`;
}

function _valTestCard(id, title, desc, threshold, btnLabel, estTime) {
  const have = threshold?.have ?? 0;
  const need = threshold?.need ?? 0;
  const canRun = have >= need;
  const pct = need > 0 ? Math.min(100, Math.round(have / need * 100)) : 100;
  const barColor = canRun ? 'var(--green)' : 'var(--yellow)';
  const cached = _valTestResults[id];
  const badge = cached ? _valBadgeFor(id, cached) : (canRun ? '<span class="val-test-badge ready">READY</span>' : '<span class="val-test-badge nodata">NEED DATA</span>');
  const collapsed = cached ? '' : ' collapsed';

  let bodyHtml = `<div class="val-desc">${desc}</div>`;
  const shortfall = need - have;

  // Data requirement bar
  if (need > 0) {
    const rate = _valSummary?.daily_rate || 0;
    let etaNote = '';
    if (!canRun && rate > 0) {
      const days = Math.ceil(shortfall / rate);
      etaNote = ` · ~${days} day${days!==1?'s':''} at current rate`;
    }
    bodyHtml += `<div class="val-req-bar">
      <div class="val-req-label">
        <span>${threshold?.label || 'observations'}: ${have}/${need}${canRun ? ' ✓' : ''}${etaNote}</span>
      </div>
      <div class="val-req-track"><div class="val-req-fill" style="width:${pct}%;background:${barColor}"></div></div>
    </div>`;
  }

  // Button or results
  bodyHtml += `<div id="valResult_${id}">`;
  if (cached) {
    bodyHtml += _valRenderResult(id, cached);
  } else {
    bodyHtml += `<button class="val-run-btn" id="valBtn_${id}" onclick="_valRunTest('${id}')" ${canRun ? '' : 'disabled'}>
      ${canRun ? `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"/></svg> ${btnLabel}` : `Need ${shortfall || need} more ${threshold?.label || 'observations'}`}
    </button>`;
    if (canRun && estTime) {
      bodyHtml += `<div style="font-size:9px;color:var(--dim);text-align:center;margin-top:4px">Estimated: ${estTime}</div>`;
    }
  }
  bodyHtml += '</div>';

  return `<div class="val-test-card${collapsed}" id="valCard_${id}">
    <div class="val-test-header" onclick="_valToggle('${id}')">
      <div style="display:flex;align-items:center;gap:8px">
        <div class="val-test-title">${title}</div>
        ${badge}
      </div>
      <span class="vtc-arrow">▾</span>
    </div>
    <div class="val-test-body">${bodyHtml}</div>
  </div>`;
}

function _valToggle(id) {
  const card = document.getElementById('valCard_' + id);
  if (card) card.classList.toggle('collapsed');
}

function _valBadgeFor(id, result) {
  if (!result) return '';
  if (result._error) return '<span class="val-test-badge fail">ERROR</span>';

  switch(id) {
    case 'persistence': {
      const v = result.verdict;
      if (v === 'strong') return '<span class="val-test-badge pass">STRONG</span>';
      if (v === 'weak') return '<span class="val-test-badge" style="background:rgba(210,153,34,0.12);color:var(--yellow)">WEAK</span>';
      if (v === 'poor') return '<span class="val-test-badge fail">POOR</span>';
      return '<span class="val-test-badge nodata">NO DATA</span>';
    }
    case 'permutation': {
      const p = result.p_value;
      if (p != null && p < 0.05) return '<span class="val-test-badge pass">SIGNIFICANT</span>';
      if (p != null && p < 0.10) return '<span class="val-test-badge" style="background:rgba(210,153,34,0.12);color:var(--yellow)">MARGINAL</span>';
      if (p != null) return '<span class="val-test-badge fail">NOT SIG</span>';
      return '<span class="val-test-badge nodata">NO DATA</span>';
    }
    case 'walkforward': {
      const v = result.verdict;
      if (v === 'selection_works') return '<span class="val-test-badge pass">WORKS</span>';
      if (v === 'selection_unreliable') return '<span class="val-test-badge fail">UNRELIABLE</span>';
      return '<span class="val-test-badge nodata">NO DATA</span>';
    }
    case 'realized_edge': {
      ...
      if (w) {
        ...
        if (first && first.avg_pnl_per_contract_c >= 0) return '<span class="val-test-badge pass">POSITIVE</span>';
        if (first) return '<span class="val-test-badge fail">NEGATIVE</span>';
      }
      return '<span class="val-test-badge nodata">NO DATA</span>';
    }
    case 'pnl_attribution': return '<span class="val-test-badge pass">LOADED</span>';
    case 'shadow': return '<span class="val-test-badge pass">LOADED</span>';
    case 'capture_rate': {
      const pct = result.capture_rate_pct;
      if (pct >= 90) return '<span class="val-test-badge pass">HEALTHY</span>';
      if (pct >= 70) return `<span class="val-test-badge" style="background:rgba(210,153,34,0.12);color:var(--yellow)">${pct}%</span>`;
      return `<span class="val-test-badge fail">${pct || 0}%</span>`;
    }
    default: return '<span class="val-test-badge pass">DONE</span>';
  }
}

async function _valResetSelection() {
  try {
    const r = await api('/api/walkforward_selection_reset', {
      method: 'POST',
      headers: {'Content-Type': 'application/json', 'X-CSRF-Token': _getCsrfToken()},
    });
    if (r.ok || r.message) {
      showToast(r.message || 'Selection test cleared', 'green');
      // Refresh the validation page
      setTimeout(() => statsNavTo('validation'), 500);
    } else {
      showToast(r.error || 'Failed to clear', 'red');
    }
  } catch(e) {
...
async function _valRunTest(id) {
  const btn = document.getElementById('valBtn_' + id);
  const resultEl = document.getElementById('valResult_' + id);
  const card = document.getElementById('valCard_' + id);

  // Show running state
  if (btn) {
    btn.classList.add('running');
    btn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation:spin 1s linear infinite"><path d="M21 12a9 9 0 11-6.219-8.56"/></svg> Running...';
  }
  // Add progress bar
  let progressHtml = '<div class="val-progress-bar"><div class="val-progress-fill indeterminate"></div></div>';
  if (btn) btn.insertAdjacentHTML('afterend', progressHtml);

  try {
    let result;
    // Heavy tests route through bot command queue (no nginx timeout)
    const _heavyTests = ['walkforward', 'permutation', 'persistence'];
    if (_heavyTests.includes(id)) {
      // Clear previous result
      await api(`/api/validation_result/${id}/clear`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json', 'X-CSRF-Token': _getCsrfToken()},
      }).catch(() => {});
      // Send command to bot
      const _cmdParams = {test_id: id};
      if (id === 'permutation') _cmdParams.n = 500;
      if (id === 'walkforward') _cmdParams.folds = 5;
      await cmd('run_validation_test', _cmdParams);
      // Poll for result (bot stores it when done)
      const _pollStart = Date.now();
      const _maxWait = 300000; // 5 min max
      while (Date.now() - _pollStart < _maxWait) {
        await new Promise(r => setTimeout(r, 3000));
        const _pr = await api(`/api/validation_result/${id}`).catch(() => null);
        if (_pr && !_pr.pending) {
          result = _pr;
          break;
        }
        // Update progress indicator with elapsed time
        if (btn) {
          const _elapsed = Math.round((Date.now() - _pollStart) / 1000);
          btn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation:spin 1s linear infinite"><path d="M21 12a9 9 0 11-6.219-8.56"/></svg> Running ${_elapsed}s...`;
        }
      }
      if (!result) {
        result = {_error: 'Test timed out after 5 minutes. Check bot logs.'};
      }
    } else {
      switch(id) {
        case 'realized_edge':
          result = await api('/api/realized_edge');
          break;
        case 'pnl_attribution':
          result = await api('/api/pnl_attribution');
          break;
        case 'shadow': {
          const [sa, sr] = await Promise.all([
            api('/api/shadow_analysis').catch(() => null),
            api('/api/shadow_reconciliation').catch(() => null),
          ]);
          result = {_shadow_analysis: sa, _reconciliation: sr};
          break;
        }
        case 'capture_rate':
          result = await api('/api/capture_rate');
          break;
      }
    }
    ...
    _valTestResults[id] = result;
    resultEl.innerHTML = _valRenderResult(id, result);

    // Update badge in header
    const header = card.querySelector('.val-test-header');
    const oldBadge = header.querySelector('.val-test-badge');
    if (oldBadge) oldBadge.outerHTML = _valBadgeFor(id, result);

  } catch(e) {
    const errResult = {_error: e.message};
    _valTestResults[id] = errResult;
    resultEl.innerHTML = `<div style="padding:10px;border:1px solid var(--red);border-radius:6px;background:rgba(248,81,73,0.06)">
      <div style="font-size:12px;color:var(--red);font-weight:600;margin-bottom:4px">Test Failed</div>
      <div style="font-size:11px;color:var(--dim)">${e.message}</div>
      <button class="val-run-btn" style="margin-top:8px" onclick="_valRunTest('${id}')">Retry</button>
    </div>`;
    const header = card.querySelector('.val-test-header');
    const oldBadge = header.querySelector('.val-test-badge');
    if (oldBadge) oldBadge.outerHTML = '<span class="val-test-badge fail">ERROR</span>';
  } finally {
    // Clean up progress bar and button state
    if (btn) {
      btn.classList.remove('running');
      btn.innerHTML = 'Re-run';
      const pb = btn.parentElement?.querySelector('.val-progress-bar');
      if (pb) pb.remove();
    }
  }
}

function _valRenderResult(id, result) {
  if (!result) return '<div class="dim">No data</div>';
  if (result._error) {
    return `<div style="padding:10px;border:1px solid var(--red);border-radius:6px;background:rgba(248,81,73,0.06)">
      <div style="font-size:12px;color:var(--red);font-weight:600;margin-bottom:4px">Error</div>
      <div style="font-size:11px;color:var(--dim)">${result._error}</div>
      <button class="val-run-btn" style="margin-top:8px" onclick="_valRunTest('${id}')">Retry</button>
    </div>`;
  }

  switch(id) {
    case 'persistence': return _valRenderPersistence(result);
    case 'permutation': return _valRenderPermutation(result);
    case 'walkforward': return _valRenderWalkforward(result);
    case 'realized_edge': return _valRenderRealizedEdge(result);
    case 'pnl_attribution': return _valRenderPnlAttribution(result);
    case 'shadow': return _valRenderShadow(result);
    case 'capture_rate': return _valRenderCapture(result);
    default: return '<div class="dim">Unknown test</div>';
  }
}

function _valRenderPersistence(d) {
  if (d.error) return `<div class="val-result"><div class="val-result-detail" style="color:var(--red)">${d.error}</div><button class="val-run-btn" style="margin-top:8px" onclick="_valRunTest('persistence')">Retry</button></div>`;

  const v = d.verdict;
  const vColor = v === 'strong' ? 'var(--green)' : v === 'weak' ? 'var(--yellow)' : 'var(--red)';
  const vLabel = v === 'strong' ? 'Strong Persistence' : v === 'weak' ? 'Weak Persistence' : v === 'poor' ? 'Poor Persistence' : 'Insufficient Data';
  const vDesc = v === 'strong' ? 'Most winning strategies from the first half stayed winners. Your edge looks real.'
    : v === 'weak' ? 'Some strategies persisted, but not consistently. Edge may be partly real, partly noise.'
    : v === 'poor' ? 'First-half winners mostly failed in the second half. The edge may be overfitting.'
    : 'Not enough data in both halves to draw conclusions.';

  let h = `<div class="val-result">
    <div class="val-result-verdict"><span style="color:${vColor}">${vLabel}</span></div>
    <div class="val-result-detail">${vDesc}</div>
    <div class="val-result-stats">
      <div class="val-result-stat"><div class="vrs-val">${d.persist_rate != null ? Math.round(d.persist_rate*100)+'%' : '—'}</div><div class="vrs-label">Persist Rate</div></div>
      <div class="val-result-stat"><div class="vrs-val">${d.persisted ?? '—'}/${d.total_with_second_half_data ?? '—'}</div><div class="vrs-label">Persisted / Tested</div></div>
      <div class="val-result-stat"><div class="vrs-val">${d.n_first_half ?? '—'}</div><div class="vrs-label">1st Half Obs</div></div>
      <div class="val-result-stat"><div class="vrs-val">${d.n_second_half ?? '—'}</div><div class="vrs-label">2nd Half Obs</div></div>
    </div>`;

  const details = d.details || [];
  if (details.length > 0) {
    h += '<div style="margin-top:10px;font-size:10px;font-weight:600;text-transform:uppercase;color:var(--blue);margin-bottom:4px">Top Strategies: 1st → 2nd Half</div>';
    h += _sTable([{label:'Strategy',left:true},{label:'1st EV'},{label:'2nd EV'},{label:'Δ'}],
      details.map(s => {
        const ev1 = s.first_half_ev_c != null ? `${s.first_half_ev_c > 0 ? '+' : ''}${s.first_half_ev_c}¢` : '—';
        const ev2 = s.second_half_ev_c != null ? `${s.second_half_ev_c > 0 ? '+' : ''}${s.second_half_ev_c}¢` : '—';
        const chg = s.ev_change_c != null ? `${s.ev_change_c > 0 ? '+' : ''}${s.ev_change_c}¢` : '—';
        const cls1 = (s.first_half_ev_c||0) > 0 ? 'pos' : 'neg';
        const cls2 = (s.second_half_ev_c||0) > 0 ? 'pos' : 'neg';
        const clsC = (s.ev_change_c||0) > 0 ? 'pos' : 'neg';
        return `<tr><td style="text-align:left;font-size:9px;max-width:100px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(s.strategy_key||'').replace(/:/g,' · ')}</td>
          <td class="${cls1}">${ev1}</td><td class="${cls2}">${ev2}</td><td class="${clsC}">${chg}</td></tr>`;
      }).join('')
    );
  }
  h += '<button class="val-run-btn" style="margin-top:10px" onclick="_valRunTest(\'persistence\')">Re-run</button></div>';
  return h;
}

function _valRenderPermutation(d) {
  if (d.error) return `<div class="val-result"><div class="val-result-detail" style="color:var(--red)">${d.error}</div><button class="val-run-btn" style="margin-top:8px" onclick="_valRunTest('permutation')">Retry</button></div>`;

  const p = d.p_value;
  const sig = p != null && p < 0.05;
  const marginal = p != null && p >= 0.05 && p < 0.10;
  const vColor = sig ? 'var(--green)' : marginal ? 'var(--yellow)' : 'var(--red)';
  const vLabel = sig ? 'Edge Confirmed' : marginal ? 'Inconclusive' : 'No Edge Detected';
  const vDesc = sig ? 'Your best strategy\'s EV beats 95%+ of random shuffles. The edge is statistically significant — it\'s not just an artifact of testing many strategies.'
    : marginal ? 'Close to significance but not definitive. More data may push this one way or the other.'
    : 'Your best strategy didn\'t beat randomized outcomes convincingly. The apparent edge could be noise from testing thousands of variants.';

  const permBest = d.permuted_best_evs || {};
  let h = `<div class="val-result">
    <div class="val-result-verdict"><span style="color:${vColor}">p = ${p != null ? p.toFixed(3) : '—'}</span><span style="font-size:11px;color:var(--dim);font-weight:normal;margin-left:6px">${vLabel}</span></div>
    <div class="val-result-detail">${vDesc}</div>
    <div class="val-result-stats">
      <div class="val-result-stat"><div class="vrs-val ${(d.real_best_ev_c||0)>0?'pos':'neg'}">${d.real_best_ev_c != null ? (d.real_best_ev_c>0?'+':'')+d.real_best_ev_c+'¢' : '—'}</div><div class="vrs-label">Best Real EV</div></div>
      <div class="val-result-stat"><div class="vrs-val dim">${permBest.mean_c != null ? permBest.mean_c+'¢' : '—'}</div><div class="vrs-label">Mean Shuffled EV</div></div>
      <div class="val-result-stat"><div class="vrs-val">${d.n_observations ?? '—'}</div><div class="vrs-label">Observations</div></div>
      <div class="val-result-stat"><div class="vrs-val">${d.n_permutations ?? '—'}</div><div class="vrs-label">Permutations</div></div>
    </div>`;
  if (d.real_best_strategy) {
    h += `<div style="font-size:10px;color:var(--dim);margin-top:8px">Best strategy: <span style="font-family:monospace;color:var(--text)">${d.real_best_strategy.replace(/:/g,' · ')}</span></div>`;
  }
  h += '<button class="val-run-btn" style="margin-top:10px" onclick="_valRunTest(\'permutation\')">Re-run</button></div>';
  return h;
}

function _valRenderWalkforward(d) {
  if (d.error) return `<div class="val-result"><div class="val-result-detail" style="color:var(--red)">${d.error}</div><button class="val-run-btn" style="margin-top:8px" onclick="_valRunTest('walkforward')">Retry</button></div>`;

  const v = d.verdict;
  const summary = d.summary || {};
  const vColor = v === 'selection_works' ? 'var(--green)' : v === 'selection_unreliable' ? 'var(--red)' : 'var(--yellow)';
  const vLabel = v === 'selection_works' ? 'Selection Works' : v === 'selection_unreliable' ? 'Selection Unreliable' : 'Insufficient Data';
  const vDesc = v === 'selection_works' ? 'The auto-strategy advisor\'s picks were profitable out-of-sample in most folds. This means you can trust it to pick strategies for live trading.'
    : v === 'selection_unreliable' ? 'The advisor\'s top picks didn\'t hold up out-of-sample. Auto-strategy would likely lose money. The selection process needs more data or a different approach.'
    : 'Not enough data to fully evaluate the selection process yet.';

  let h = `<div class="val-result">
    <div class="val-result-verdict"><span style="color:${vColor}">${vLabel}</span></div>
    <div class="val-result-detail">${vDesc}</div>
    <div class="val-result-stats">
      <div class="val-result-stat"><div class="vrs-val">${summary.folds_profitable ?? '—'}/${summary.folds_tested ?? '—'}</div><div class="vrs-label">Profitable Folds</div></div>
      <div class="val-result-stat"><div class="vrs-val ${(summary.aggregate_test_ev_c||0)>0?'pos':'neg'}">${summary.aggregate_test_ev_c != null ? (summary.aggregate_test_ev_c>0?'+':'')+summary.aggregate_test_ev_c+'¢' : '—'}</div><div class="vrs-label">Avg OOS EV</div></div>
      <div class="val-result-stat"><div class="vrs-val">${summary.total_test_trades ?? '—'}</div><div class="vrs-label">OOS Trades</div></div>
      <div class="val-result-stat"><div class="vrs-val">${summary.selection_success_rate != null ? Math.round(summary.selection_success_rate*100)+'%' : '—'}</div><div class="vrs-label">Success Rate</div></div>
    </div>`;

  const folds = d.fold_results || [];
  if (folds.length > 0) {
    h += '<div style="margin-top:10px;font-size:10px;font-weight:600;text-transform:uppercase;color:var(--blue);margin-bottom:4px">Fold Details</div>';
    h += _sTable([{label:'Fold',left:true},{label:'Train'},{label:'OOS'},{label:'Trades'},{label:'Strategy'}],
      folds.map(f => {
        const trainEv = f.train_ev_c != null ? `${f.train_ev_c>0?'+':''}${f.train_ev_c}¢` : '—';
        const testEv = f.test_ev_c != null ? `${f.test_ev_c>0?'+':''}${f.test_ev_c}¢` : '—';
        const strat = f.selected ? f.selected.replace(/:/g,' · ') : (f.reason || 'none');
        return `<tr><td>Fold ${f.fold}</td>
          <td class="${(f.train_ev_c||0)>0?'pos':'neg'}">${trainEv}</td>
          <td class="${(f.test_ev_c||0)>0?'pos':'neg'}">${testEv}</td>
          <td>${f.test_trades ?? '—'}</td>
          <td style="font-size:9px;max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${strat}</td></tr>`;
      }).join('')
    );
  }
  h += '<button class="val-run-btn" style="margin-top:10px" onclick="_valRunTest(\'walkforward\')">Re-run</button></div>';
  return h;
}

function _valRenderRealizedEdge(d) {
  if (!d || !d.windows || Object.keys(d.windows).length === 0) {
    return `<div class="val-result">
      <div class="val-result-detail">No realized edge data yet. This requires real trades (not simulated). Once you start trading, this will compare actual P&L to what the Observatory predicted.</div>
    </div>`;
  }

  let h = '<div class="val-result">';
  h += _sTable([{label:'Window',left:true},{label:'Trades'},{label:'¢/Trade'},{label:'¢/Contract'},{label:'Win%'}],
    Object.entries(d.windows).map(([w, v]) => {
      return `<tr><td>Last ${w}</td><td>${v.n_trades}</td>
        <td class="${v.avg_pnl_per_trade_c>=0?'pos':'neg'}">${v.avg_pnl_per_trade_c>=0?'+':''}${v.avg_pnl_per_trade_c}</td>
        <td class="${v.avg_pnl_per_contract_c>=0?'pos':'neg'}">${v.avg_pnl_per_contract_c>=0?'+':''}${v.avg_pnl_per_contract_c}</td>
        <td>${(v.win_rate*100).toFixed(1)}%</td></tr>`;
    }).join('')
  );
  if (d.simulated_ev_c != null) {
    const gap = d.sim_live_gap_c;
    h += `<div style="margin-top:10px;padding:8px;border-radius:6px;background:var(--bg);font-size:12px">
      Sim EV: <strong>${d.simulated_ev_c>0?'+':''}${d.simulated_ev_c}¢</strong>
      ${gap != null ? ` · Execution gap: <span class="${Math.abs(gap)<=2?'pos':'neg'}">${gap.toFixed(1)}¢</span>` : ''}
      <div class="dim" style="font-size:10px;margin-top:2px">${(d.simulated_strategy||'').replace(/:/g,' · ')}</div>
    </div>`;
    if (gap != null) {
      const gapDesc = Math.abs(gap) <= 1 ? 'Execution is matching simulation closely — minimal cost.'
        : Math.abs(gap) <= 3 ? 'Moderate execution gap. Some edge is being lost to slippage/fees.'
        : 'Large execution gap — the strategy may look good in simulation but loses money in practice.';
      h += `<div class="val-result-detail" style="margin-top:6px">${gapDesc}</div>`;
    }
  }
  h += '<button class="val-run-btn" style="margin-top:10px" onclick="_valRunTest(\'realized_edge\')">Refresh</button></div>';
  return h;
}

function _valRenderPnlAttribution(d) {
  if (d.error) return `<div class="val-result"><div class="val-result-detail">Not enough trade data for attribution analysis. Need at least 5 real trades with outcomes.</div></div>`;

  let h = '<div class="val-result">';
  const comps = d.components || {};
  if (Object.keys(comps).length > 0) {
    h += '<div class="val-result-stats">';
    for (const [k, v] of Object.entries(comps)) {
      const label = k.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      const val = v != null ? (v > 0 ? '+' : '') + v.toFixed(2) + '¢' : '—';
      const cls = v > 0 ? 'pos' : v < 0 ? 'neg' : '';
      h += `<div class="val-result-stat"><div class="vrs-val ${cls}">${val}</div><div class="vrs-label">${label}</div></div>`;
    }
    h += '</div>';
  }
  if (d.summary) h += `<div class="val-result-detail" style="margin-top:8px">${d.summary}</div>`;
  if (Object.keys(comps).length === 0 && !d.summary) {
    h += '<div class="val-result-detail">No attribution data available. Need more real trades with outcomes.</div>';
  }
  h += '<button class="val-run-btn" style="margin-top:10px" onclick="_valRunTest(\'pnl_attribution\')">Refresh</button></div>';
  return h;
}

function _valRenderShadow(d) {
  const sa = d._shadow_analysis;
  const sr = d._reconciliation;
  const hasData = (sa && !sa.error) || (sr && !sr.error);

  if (!hasData) {
    return `<div class="val-result">
      <div class="val-result-detail">No shadow trade data yet. Enable shadow trading in Settings to place 1-contract test trades. This measures the real-world gap between simulated and actual execution.</div>
    </div>`;
  }
  ...
  if (sa && !sa.error) {
    h += '<div class="val-result-stats">';
    if (sa.avg_slippage_c != null) h += `<div class="val-result-stat"><div class="vrs-val ${sa.avg_slippage_c > 1 ? 'neg' : 'pos'}">${sa.avg_slippage_c.toFixed(2)}¢</div><div class="vrs-label">Avg Slippage</div></div>`;
    if (sa.fill_rate != null) h += `<div class="val-result-stat"><div class="vrs-val">${(sa.fill_rate*100).toFixed(1)}%</div><div class="vrs-label">Fill Rate</div></div>`;
    if (sa.n_analyzed != null) h += `<div class="val-result-stat"><div class="vrs-val">${sa.n_analyzed}</div><div class="vrs-label">Trades Analyzed</div></div>`;
    h += '</div>';
  }
  if (sr && !sr.error) {
    if (sr.systematic_gap_c != null) {
      const gapCls = Math.abs(sr.systematic_gap_c) < 2 ? 'pos' : 'neg';
      h += `<div style="margin-top:8px;padding:8px;border-radius:6px;background:var(--bg);font-size:12px">
        Systematic gap: <strong class="${gapCls}">${sr.systematic_gap_c>=0?'+':''}${sr.systematic_gap_c.toFixed(1)}¢</strong>
        ${sr.n_reconciled ? `<span class="dim"> · ${sr.n_reconciled} trades reconciled</span>` : ''}
      </div>`;
      if (sr.verdict) h += `<div class="val-result-detail" style="margin-top:6px">${sr.verdict}</div>`;
    }
  }
  h += '<button class="val-run-btn" style="margin-top:10px" onclick="_valRunTest(\'shadow\')">Refresh</button></div>';
  return h;
}

function _valRenderCapture(d) {
  if (d.error) return `<div class="val-result"><div class="val-result-detail" style="color:var(--red)">${d.error}</div></div>`;

  const pct = d.capture_rate_pct ?? 0;
  const color = pct >= 90 ? 'var(--green)' : pct >= 70 ? 'var(--yellow)' : 'var(--red)';
  const desc = pct >= 90 ? 'Excellent capture rate — your data has minimal gaps.'
    : pct >= 70 ? 'Some markets are being missed. Check for recent restarts or deploys that interrupted collection.'
    : pct >= 50 ? 'Significant gaps in data collection. The bot may have been offline or restarting frequently.'
    : 'Very low capture rate. The bot may not have been running consistently over the last 24 hours.';

  let h = `<div class="val-result">
    <div style="text-align:center;margin-bottom:8px">
      <div style="font-size:28px;font-weight:700;font-family:monospace;color:${color}">${pct}%</div>
      <div style="font-size:10px;color:var(--dim)">Last ${d.hours || 24} hours</div>
    </div>
    <div class="val-result-detail">${desc}</div>
    <div class="val-result-stats" style="margin-top:8px">
      <div class="val-result-stat"><div class="vrs-val">${d.total_captured ?? 0}</div><div class="vrs-label">Captured</div></div>
      <div class="val-result-stat"><div class="vrs-val">${d.expected_markets ?? 96}</div><div class="vrs-label">Expected</div></div>
      <div class="val-result-stat"><div class="vrs-val">${d.full_quality ?? 0}</div><div class="vrs-label">Full Quality</div></div>
      <div class="val-result-stat"><div class="vrs-val ${(d.partial||0) + (d.few||0) > 0 ? 'neg' : ''}">${(d.partial||0) + (d.few||0)}</div><div class="vrs-label">Partial/Few</div></div>
    </div>`;
  h += '<button class="val-run-btn" style="margin-top:10px" onclick="_valRunTest(\'capture_rate\')">Refresh</button></div>';
  return h;
}


// ────────────────────────────────────────────────────
//  DATA CONVERGENCE PAGE
// ────────────────────────────────────────────────────
var _convHours = 24;  // default timeframe

async function _statsRenderConvergence(el) {
  el.innerHTML = '<div class="dim" style="padding:20px 0;text-align:center">Loading...</div>';
  const d = await api(`/api/data_convergence?hours=${_convHours}`).catch(e => ({error: e.message, has_data: false}));
  let html = '';

  if (!d.has_data) {
    html += `<div style="text-align:center;padding:40px 16px">
      <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--dim)" stroke-width="1.5" style="margin-bottom:12px;opacity:0.5"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>
      <div style="font-size:13px;color:var(--dim);line-height:1.5">${d.error || 'No convergence data yet.'}</div>
      <div style="font-size:11px;color:var(--dim);margin-top:8px;opacity:0.7">Snapshots are recorded every ~30 minutes when the Observatory runs simulations. Check back after a couple of cycles.</div>
    </div>`;
    el.innerHTML = html;
    return;
  }
  ...
  const color = overall >= 85 ? 'var(--green)' : overall >= 60 ? 'var(--yellow)' : 'var(--blue)';
  const verdict = overall >= 90 ? 'Converged' : overall >= 75 ? 'Stabilizing' : overall >= 50 ? 'Settling' : 'Changing Fast';
  const verdictSub = overall >= 90 ? 'Numbers have mostly stopped moving — more data won\'t change much'
    : overall >= 75 ? 'Key metrics are settling but still shifting slightly'
    : overall >= 50 ? 'Data is still actively changing the picture'
    : 'Numbers are moving significantly — keep collecting';

  // ═══ Main stability ring ═══
  const r = 46, circ = 2 * Math.PI * r;
  const offset = circ - (overall / 100) * circ;
  html += `<div class="val-readiness" style="padding:20px 0 8px">
    <div class="val-ring" style="width:108px;height:108px">
      <svg width="108" height="108" viewBox="0 0 108 108">
        <circle cx="54" cy="54" r="${r}" fill="none" stroke="var(--border)" stroke-width="7"/>
        <circle cx="54" cy="54" r="${r}" fill="none" stroke="${color}" stroke-width="7"
          stroke-dasharray="${circ.toFixed(1)}" stroke-dashoffset="${offset.toFixed(1)}" stroke-linecap="round"
          style="transition:stroke-dashoffset 0.6s ease"/>
      </svg>
      <div class="val-ring-label" style="color:${color};font-size:30px">${overall}%</div>
    </div>
    <div class="val-verdict" style="color:${color};font-size:15px;margin-top:8px">${verdict}</div>
    <div class="val-sub" style="max-width:260px;margin:4px auto 0">${verdictSub}</div>
  </div>`;

  // ═══ Timeframe pills ═══
  const tfs = [{h:6,l:'6h'},{h:12,l:'12h'},{h:24,l:'24h'},{h:48,l:'2d'},{h:72,l:'3d'},{h:168,l:'7d'}];
  html += '<div style="display:flex;justify-content:center;gap:6px;margin:12px 0 16px">';
  for (const tf of tfs) {
    const active = tf.h === _convHours;
    html += `<button onclick="_convSetHours(${tf.h})" style="
      padding:4px 12px;border-radius:12px;font-size:11px;font-weight:${active?'700':'500'};
      border:1px solid ${active?color:'var(--border)'};
      background:${active?color:'none'};color:${active?'#000':'var(--dim)'};
      cursor:pointer;-webkit-tap-highlight-color:transparent">${tf.l}</button>`;
  }
  html += '</div>';

  // ═══ Observation context ═══
  html += `<div style="display:flex;justify-content:center;gap:16px;margin-bottom:16px;font-size:11px;color:var(--dim)">
    <span>${d.obs_previous || '?'} → <strong style="color:var(--text)">${d.obs_current || '?'}</strong> obs</span>
    <span style="color:${color}">+${d.obs_delta || 0} (${d.obs_growth_pct > 0 ? '+' : ''}${d.obs_growth_pct || 0}%)</span>
  </div>`;

  // ═══ Category cards ═══
  const cats = d.categories || {};
  const catOrder = ['strategy', 'edge', 'dimensions', 'execution'];
  const catIcons = {
    strategy: '<path d="M12 20V10M18 20V4M6 20v-4"/>',
    edge: '<path d="M13 2L3 14h9l-1 8 10-12h-9l1-8"/>',
    dimensions: '<rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/>',
    execution: '<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>',
  };
  ...
  for (const key of catOrder) {
    ...
    if (!cat) continue;
    const s = cat.score || 0;
    const catColor = s >= 85 ? 'var(--green)' : s >= 60 ? 'var(--yellow)' : s >= 30 ? 'var(--blue)' : 'var(--red)';
    const catLabel = s >= 85 ? 'Stable' : s >= 60 ? 'Settling' : s >= 30 ? 'Shifting' : 'Volatile';
    ...
    html += `<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:8px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
        <div style="display:flex;align-items:center;gap:8px">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="${catColor}" stroke-width="2">${catIcons[key]}</svg>
          <div>
            <div style="font-size:12px;font-weight:600">${cat.label}</div>
            <div style="font-size:9px;color:var(--dim)">${cat.weight} of overall</div>
          </div>
        </div>
        <div style="text-align:right">
          <div style="font-size:20px;font-weight:700;font-family:monospace;color:${catColor}">${s}%</div>
          <div style="font-size:9px;color:${catColor}">${catLabel}</div>
        </div>
      </div>
      <div style="height:4px;background:var(--border);border-radius:2px;overflow:hidden;margin-bottom:6px">
        <div style="height:100%;width:${s}%;background:${catColor};border-radius:2px;transition:width 0.4s ease"></div>
      </div>
      ${details.length > 0 ? `<div style="font-size:10px;color:var(--dim);line-height:1.6">${details.join(' · ')}</div>` : ''}
    </div>`;
  }

  // ═══ How to read this ═══
  html += `<div style="margin-top:12px;padding:10px 12px;background:var(--card);border:1px solid var(--border);border-radius:8px">
    <div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;color:var(--dim);margin-bottom:4px">How to read this</div>
    <div style="font-size:10px;color:var(--dim);line-height:1.6">
      <strong style="color:var(--green)">90%+</strong> = Numbers have stopped moving. More data won't change the picture.<br>
      <strong style="color:var(--yellow)">60-90%</strong> = Settling down but still some movement. Keep collecting.<br>
      <strong style="color:var(--blue)">30-60%</strong> = Still actively changing. Important patterns are still emerging.<br>
      <strong style="color:var(--red)">&lt;30%</strong> = Changing fast. Very early — everything is still in flux.
    </div>
  </div>`;
...
function _convSetHours(h) {
  _convHours = h;
  const el = document.getElementById('statsSubContent');
  if (el) _statsRenderConvergence(el);
}
...
function _showChatOverlay() {
  ...
  const hdr = document.getElementById('stickyHeader');
  const tb = document.querySelector('.tab-bar');
  overlay.style.top = (hdr ? hdr.offsetHeight : 58) + 'px';
  overlay.style.bottom = (tb ? tb.offsetHeight : 76) + 'px';
  overlay.style.display = 'block';
}

function _hideChatOverlay() {
  const overlay = document.getElementById('chatOverlay');
...
// Arcade fallbacks — game.py script tag defines real versions before this runs
if (typeof window._mgS === 'undefined') {
  // game.py not loaded — provide no-ops so switchTab doesn't throw
  window.loadArcade = window.loadArcade || function(){};
  window.mgStopLoop = window.mgStopLoop || function(){};
}
...
async function doDeploy() {
  ...
  try {
    ...
    if (data.uploaded && data.uploaded.length && (!data.errors || !data.errors.length)) {
      ...
      const dashOnly = data.uploaded.every(f => f === 'game.py' || f === 'dashboard.py');
      const svcs = dashOnly ? ['kalshi-dashboard'] : ['kalshi-bot', 'kalshi-dashboard'];
...
function clearChat() {
  _chatHistory = [];
  _chatUsage = {input: 0, output: 0, msgs: 0};
  _updateChatUsage();
  $('#chatUsageBar').style.display = 'none';
...
// ── Report Generation ─────────────────────────────────────
const _reportKeywords = /\b(generate|create|make|build|produce).{0,15}(report|pdf|chart)\b|\breport\b.{0,10}(generate|create|make)/i;
...
function triggerReport() {
  ...
  // User bubble
  container.innerHTML += `<div class="chat-msg chat-user"><div>Generate a report</div><button class="chat-copy" onclick="copyMsg(this)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button></div>`;
...
function _showReportChoices() {
  const container = $('#chatMessages');
  const choiceId = 'reportChoice_' + Date.now();
  container.innerHTML += `<div class="chat-msg chat-ai" id="${choiceId}"><div>
    <div style="margin-bottom:6px">What format would you like?</div>
    <div class="chat-action-bar">
      <button class="chat-action-btn" onclick="_doReport('text','${choiceId}')">
      ...
      <button class="chat-action-btn" onclick="_doReport('chart','${choiceId}')">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 20V10"/><path d="M12 20V4"/><path d="M6 20v-6"/></svg>
        Chart
      </button>
      <button class="chat-action-btn" onclick="_doReport('pdf','${choiceId}')">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/></svg>
        Full PDF
      </button>
    </div>
    <div style="font-size:10px;color:var(--dim);margin-top:4px">Chart & PDF require matplotlib. Email <em>pip: matplotlib</em> to install.</div>
  </div></div>`;
}

async function _doReport(type, choiceId) {
  // Replace choice buttons with loading
  const choiceEl = document.getElementById(choiceId);
  const typeLabel = {text: 'Text Report', chart: 'Chart', pdf: 'Full PDF'}[type];
  if (choiceEl) {
    choiceEl.innerHTML = `<div>
      <div style="margin-bottom:6px">Generating ${typeLabel}...</div>
      <div><span class="chat-dot"></span><span class="chat-dot"></span><span class="chat-dot"></span></div>
    </div>`;
  }
  ...
  try {
    const resp = await api('/api/report/' + type, {
      method: 'POST',
      headers:{'Content-Type':'application/json','X-CSRF-Token':_getCsrfToken()},
      body: '{}',
    });
    ...
    if (resp.error) {
      if (choiceEl) choiceEl.innerHTML = `<div style="color:var(--red)">${escHtml(resp.error)}</div>`;
      return;
    }
    ...
    if (type === 'text') {
      ...
      if (choiceEl) choiceEl.innerHTML = `<div>
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
          <span style="font-weight:600;font-size:12px">Text Report</span>
          <button class="chat-action-btn" style="padding:4px 10px;font-size:11px" onclick="_copyReport(this)">
        ...
        <div class="chat-report-card"><pre id="reportText_${Date.now()}">${escaped}</pre></div>
      </div>`;
    } else if (type === 'chart') {
      const url = resp.url;
      if (choiceEl) choiceEl.innerHTML = `<div>
        <div style="font-weight:600;font-size:12px;margin-bottom:6px">Performance Chart</div>
        <img src="${url}" style="width:100%;border-radius:6px;border:1px solid var(--border);cursor:pointer" onclick="openLightbox('${url}')">
        <div style="display:flex;gap:8px;margin-top:8px">
          <button class="chat-action-btn" onclick="openLightbox('${url}')">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:14px;height:14px"><path d="M15 3h6v6M14 10l6.1-6.1M9 21H3v-6M10 14l-6.1 6.1"/></svg>
            View Full
          </button>
          <button class="chat-action-btn" onclick="shareFile('${url}','chart.png')">
      ...
      const url = resp.url;
      if (choiceEl) choiceEl.innerHTML = `<div>
        <div style="font-weight:600;font-size:12px;margin-bottom:6px">PDF Report (3 pages)</div>
        <div class="dim" style="font-size:11px;margin-bottom:8px">Summary stats, trade P&L chart, and regime performance.</div>
        <button class="chat-action-btn" onclick="shareFile('${url}','report.pdf')">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:14px;height:14px"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg>
          Open / Share PDF
        </button>
      </div>`;
    ...
    if (choiceEl) choiceEl.innerHTML = `<div style="color:var(--red)">${escHtml(e.toString())}</div>`;
  }

  container.scrollTop = container.scrollHeight;
}

function _copyReport(btn) {
  const card = btn.closest('.chat-msg').querySelector('pre');
  if (!card) return;
  copyToClipboard(card.textContent).then(() => {
...
function copyMsg(btn) {
  ...
  copyToClipboard(text).then(() => {
    btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="var(--green)" stroke-width="2" style="width:14px;height:14px"><path d="M20 6L9 17l-5-5"/></svg>';
...
function copyEntireChat() {
  ...
  msgs.forEach(m => {
    ...
    const isAi = m.classList.contains('chat-ai');
    const isErr = m.classList.contains('chat-err');
    if (!isUser && !isAi && !isErr) return;
    const bubble = m.querySelector('div');
    if (!bubble) return;
    // Use innerText for proper whitespace, decode any encoded entities
    let text = (bubble.innerText || bubble.textContent || '').trim();
    try { text = decodeURIComponent(text); } catch(e) {}
    if (!text) return;
    const prefix = isUser ? 'You' : isErr ? 'Error' : 'AI';
...
let _chatUsage = {input: 0, output: 0, msgs: 0};

function _updateChatUsage() {
  const headerEl = $('#chatUsageInfo');
  const barEl = $('#chatUsageBar');
  if (_chatUsage.msgs === 0) {
    headerEl.textContent = '';
    barEl.style.display = 'none';
    return;
  }
  // Sonnet: $3/M input, $15/M output
  const cost = (_chatUsage.input * 3 + _chatUsage.output * 15) / 1_000_000;
  // Context grows each round (~2k system + accumulated history)
  // Estimate tokens per msg: use actual average but weight recent msgs more
  const avgIn = _chatUsage.input / _chatUsage.msgs;
  const avgOut = _chatUsage.output / _chatUsage.msgs;
  // Budget: ~$5 credit default — context grows so each msg costs more
  const budget = 5.00;
  const remaining = Math.max(0, budget - cost);
  // Next msg estimate: input grows by ~avgOut (prior response) + ~200 (new question)
  const nextMsgInEst = avgIn + avgOut + 200;
  const nextMsgCost = (nextMsgInEst * 3 + avgOut * 15) / 1_000_000;
  const estLeft = nextMsgCost > 0 ? Math.max(0, Math.floor(remaining / nextMsgCost)) : 99;

  headerEl.textContent = `$${cost.toFixed(3)}`;
  barEl.style.display = '';
  const tokK = ((_chatUsage.input + _chatUsage.output) / 1000).toFixed(1);
  barEl.innerHTML = `${_chatUsage.msgs} msg${_chatUsage.msgs>1?'s':''} · ${tokK}k tokens · $${cost.toFixed(3)} used · <span style="color:${estLeft <= 3 ? 'var(--red)' : estLeft <= 8 ? 'var(--yellow)' : 'var(--dim)'}">~${estLeft} left</span>`;
}
...
async function sendChat() {
  ...
  if (_reportKeywords.test(msg)) {
    input.value = '';
    const container = $('#chatMessages');
    const welcome = document.getElementById('chatWelcome');
    if (welcome) welcome.remove();
    container.innerHTML += `<div class="chat-msg chat-user"><div>${escHtml(msg)}</div><button class="chat-copy" onclick="copyMsg(this)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button></div>`;
    _showReportChoices();
    container.scrollTop = container.scrollHeight;
    return;
  }
  ...
  try {
    ...
    if (resp.error) {
      ...
      if (thinkEl) { thinkEl.className = 'chat-msg chat-ai'; thinkEl.innerHTML = `<div>${html}</div><button class="chat-copy" onclick="copyMsg(this)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button>`; }
      // Add assistant response to history
      _chatHistory.push({role: 'assistant', content: resp.response});
      // Track usage
      if (resp.usage) {
        _chatUsage.input += resp.usage.input_tokens || 0;
        _chatUsage.output += resp.usage.output_tokens || 0;
        _chatUsage.msgs++;
        _updateChatUsage();
      }
    }
...
function switchTab(tab) {
  ...
  if (_currentTab === 'Chat') _hideChatOverlay();
  if (_currentTab === 'Arcade') try { mgStopLoop(); } catch(e) {}
  ...
  else if (tab === 'Bitcoin') { loadBtcChart(); loadRegimes(); loadRegimeWorkerStatus(); }
  else if (tab === 'Trades') loadTrades();
  else if (tab === 'Chat') _showChatOverlay();
  else if (tab === 'Settings') { loadConfig(); loadBackupInfo(); loadSvcStatus(); loadSystemStats(); }
  else if (tab === 'Arcade') { try { loadArcade(); } catch(e) {} }
}
...
// Initialize session stats from trade history (survives page refresh)
async function _initSessionStats() {
  try {
    // Wait for first state to be available
    let attempts = 0;
    while (!_uiState.status && attempts < 20) {
      await new Promise(r => setTimeout(r, 200));
      attempts++;
    }
    const sw = _uiState.session_wins || 0;
    const sl = _uiState.session_losses || 0;
    const sessionTotal = sw + sl;
    if (sessionTotal === 0) return;

    // Fetch enough trades to cover the session
    const trades = await api(`/api/trades?limit=${Math.max(sessionTotal + 5, 20)}`);
    if (!trades || !trades.length) return;

    // Filter to only completed trades (win/loss), walk oldest-first
    const completed = trades
      .filter(t => t.outcome === 'win' || t.outcome === 'loss')
      .reverse()
      .slice(-sessionTotal);  // take the last sessionTotal trades
    ...
    for (const t of completed) {
      ...
      // Streaks
      if (thisType === curType) { curLen++; }
      else { curType = thisType; curLen = 1; }
      if (won) maxWin = Math.max(maxWin, curLen);
      else maxLoss = Math.max(maxLoss, curLen);

      // Running P&L for peak/drawdown
      runPnl += (t.pnl || 0);
      if (runPnl > peak) peak = runPnl;

      // R1 wins
      // r1 wins tracking removed
    }

    _sessTrack.maxWinStreak = maxWin;
    _sessTrack.maxLossStreak = maxLoss;
    _sessTrack.curStreakType = curType;
    _sessTrack.curStreakLen = curLen;
    _sessTrack.peakPnl = peak;
    _sessTrack.prevWins = sw;
    _sessTrack.prevLosses = sl;

    // Re-render with computed stats
    renderUI(_uiState);
  } catch(e) { console.error('Session stats init error:', e); }
}
...
try {
  ...
  if (savedTab && savedTab !== 'Home') {
    const tab = savedTab === 'Regimes' ? 'Bitcoin' : savedTab;
...
function schedulePoll() {
  setTimeout(async () => {
    await pollState();
    _pollRate = _uiState.cashing_out ? 500 : 1000;
...
async function runCmd() {
  ...
  if (cmd.toLowerCase() === 'help') {
    output.innerHTML = `<pre style="color:var(--text)">` +
      ...
      `  Services: kalshi-bot, kalshi-dashboard\n\n` +
...
def _start_email_deploy():
    ...
    def run_service_commands(body_text):
        ...
        results = []
        allowed_services = {"bot": "kalshi-bot", "dashboard": "kalshi-dashboard",
                           "kalshi-bot": "kalshi-bot", "kalshi-dashboard": "kalshi-dashboard",
        ...
        for line in body_text.splitlines():
            ...
            for prefix in ['restart:', 'start:', 'stop:']:
                if line.startswith(prefix):
                    ...
                    targets = [svc_name] if svc_name else ["kalshi-bot", "kalshi-dashboard"]
                    for svc in targets:
...
def _start_health_check():
    ...
    def _hc_log(level, msg):
        try:
            insert_log(level, f"[HealthCheck] {msg}")
        except Exception:
    ...
    def check_loop():
        ...
        while True:
            try:
                enabled = get_config("health_check_enabled", False)
                if not enabled:
                    _alerted = False
                    _alert_time = None
                    _time.sleep(60)
                ...
                timeout_min = get_config("health_check_timeout_min", 5) or 5
...
def _fix_supervisor_config():
    ...
    Checks both kalshi-bot and kalshi-dashboard configs for autorestart=true.
...
def _install_watchdog_cron():
    ...
    cron_marker = "# kalshi-bot-watchdog"

    # Create watchdog script
    script_content = f"""#!/bin/bash
# Auto-generated watchdog for kalshi-bot
# Checks if the bot process is running and restarts if not
BOT_LOG="{bot_dir}/bot.log"
if ! supervisorctl status kalshi-bot | grep -q RUNNING; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') [Watchdog] Bot not running — restarting" >> "$BOT_LOG"
    supervisorctl start kalshi-bot >> "$BOT_LOG" 2>&1
fi
if ! supervisorctl status kalshi-dashboard | grep -q RUNNING; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') [Watchdog] Dashboard not running — restarting" >> "$BOT_LOG"
    supervisorctl start kalshi-dashboard >> "$BOT_LOG" 2>&1
fi
"""
    try:
