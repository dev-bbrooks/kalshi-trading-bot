...
from plugins.btc_15m.market_db import (
    ...
    get_strategy_for_setup, upsert_surface_cell,
    upsert_feature_importance, get_btc_surface_data,
)
...
# Strategy dimension definitions — 5c increments, absolute sell targets
...
def _valid_sell_targets(entry_max: int) -> list:
    ...
    targets = list(range(entry_max + 5, 100, 5))  # 5c steps above entry
...
def run_simulation_batch(limit: int = 0):
    ...
    - Slippage sensitivity for positive-EV strategies (+1c, +2c)
    ...
    # Accumulate all results per setup x strategy from scratch
...
def _run_slippage_sensitivity(observations: list, accum: dict):
    ...
    Computes EV at +1c and +2c additional slippage from existing accum data.
    ...
    Any strategy that goes negative at +1c slippage is fragile —
...
def _check_strategy_discoveries():
    ...
    """
    from plugins.btc_15m.notifications import notify_strategy_discovery
    ...
    for regime, r in best_per_regime.items():
        if regime not in notified:
            ...
            log.info(f"Strategy discovery: {regime} -> "
                     f"{r['strategy_key']} EV {r['ev_per_trade_c']:+.1f}c")
...
def _check_global_best_change():
    ...
    """
    from plugins.btc_15m.notifications import notify_global_best_changed
    ...
    if current_key != last_key and last_key:
        ...
        log.info(f"Global best changed: {last_key} -> {current_key} "
                 f"(EV {best['ev_per_trade_c']:+.1f}c, n={best['sample_size']})")
...
def _write_strategy_result(setup_key, setup_type, strat_key, meta, trades):
    ...
    # Analytical approximation: if fee increases by delta, cost per contract rises
    # by ~entry_price * delta, so EV drops by ~entry_price * delta.
...
def _apply_fdr_correction():
    ...
    With 2500+ strategy variants x multiple setup dimensions, some will appear
    ...
    # BH procedure: find the largest i where p(i) <= (i/m) * alpha
    bh_threshold_idx = -1
    ...
    if sig_count > 0:
        log.info(f"FDR correction: {sig_count}/{m} strategies significant at alpha={alpha}")
...
class BtcFairValueModel:
    ...
    That probability IS the fair value of a YES contract (x 100 = cents).
    ...
    def _interpolate(self, dist_pct: float, secs: float):
        ...
        for d_off in (-1, 0, 1):
            for t_off in (-1, 0, 1):
                ...
                # Weight: inverse distance x sample depth
    ...
    def _analytical_estimate(self, dist_pct: float, secs: float,
        ...
        sigma calibrated from realized vol or defaults to ~0.1%/sqrt(min) for BTC.
        """
        time_remaining = max(1, 900 - secs)
        minutes_remaining = time_remaining / 60

        # sigma per minute in percent; default ~0.10%, use realized_vol if available
        sigma_per_min = 0.10
        if realized_vol is not None and realized_vol > 0:
...
def get_recommendation(regime_label: str, hour_et: int = None,
    ...
    Validation gates (all must pass for a strategy to be recommended):
      ...
      4. Slippage robustness: flagged if +1c slippage turns EV negative
        ...
        for best in strategies:
            ...
            if ev is None or ev <= 0:
                _track_rejection(3, "ev_negative",
                                 f"EV {(ev or 0):+.1f}c <= 0",
                                 f"Best EV {(ev or 0):+.1f}c not positive ({setup_key})",
            ...
            if ev <= fee_impact_c:
                _track_rejection(4, "fee_fragile",
                                 f"EV {ev:+.1f}c <= fee impact {fee_impact_c}c",
                                 f"Fee fragile: EV {ev:+.1f}c <= 1% fee bump ({fee_impact_c}c) for {_sk} ({setup_key})",
                                 setup=setup_key, strat=_sk)
                continue
            ...
            if oos_ev <= 0:
                ...
                _track_rejection(6, "oos_negative",
                                 f"OOS EV {oos_ev:+.1f}c <= 0",
                                 f"OOS EV negative: {oos_ev:+.1f}c for {_sk} ({setup_key}) -- likely overfit",
                                 setup=setup_key, strat=_sk)
                continue
...
def compare_hold_vs_sell(setup_key: str = "global:all",
    ...
    For each side_rule x timing x entry_max combination, compares the
...
def run_permutation_test(setup_key: str = "global:all",
    ...
    log.info(f"Permutation test: real best EV={real_best_ev:+.1f}c, "
...
def _record_convergence_snapshot():
    ...
    # Store as a config entry with timestamp (simple approach when insert_metric_snapshot
    # is not available in the platform db layer)
    try:
        set_config("_convergence_snapshot", json.dumps(metrics))
        set_config("_convergence_snapshot_at", now_utc())
    except Exception:
        pass

    log.debug(f"Convergence snapshot: obs={total_obs}, best_ev={metrics['best_ev_c']:+.1f}c, "
